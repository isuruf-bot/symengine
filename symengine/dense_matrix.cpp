...
void DenseMatrix::add_matrix(const MatrixBase &other,
...
void DenseMatrix::mul_matrix(const MatrixBase &other,
                             MatrixBase &result) const {
...
void DenseMatrix::submatrix(MatrixBase &result, unsigned row_start,
  ...
  if (is_a<DenseMatrix>(result)) {
    DenseMatrix &r = down_cast<DenseMatrix &>(result);
    submatrix_dense(*this, r, row_start, col_start, row_end, col_end, row_step,
                    col_step);
  }
}
...
void sjacobian(const DenseMatrix &A, const DenseMatrix &x,
               DenseMatrix &result) {
  SYMENGINE_ASSERT(A.col_ == 1);
  SYMENGINE_ASSERT(x.col_ == 1);
  SYMENGINE_ASSERT(A.row_ == result.nrows() and x.row_ == result.ncols());
    ...
    for (unsigned j = 0; j < result.col_; j++) {
      if (is_a<Symbol>(*(x.m_[j]))) {
        ...
        result.m_[i * result.col_ + j] =
            ssubs(ssubs(A.m_[i], {{x.m_[j], x_}})->diff(x_), {{x_, x.m_[j]}});
      }
    }
  }
}
    ...
    for (unsigned j = 0; j < result.col_; j++) {
      if (is_a<Symbol>(*x)) {
        ...
        result.m_[i * result.col_ + j] = ssubs(
            ssubs(A.m_[i * result.col_ + j], {{x, x_}})->diff(x_), {{x_, x}});
      }
    }
  }
}
...
// ------------------------------- Matrix Addition ---------------------------//
void add_dense_dense(const DenseMatrix &A, const DenseMatrix &B,
                     DenseMatrix &C) {
...
// ---------------------------- Joining Operations ---------------------------//
void DenseMatrix::row_join(const DenseMatrix &B) { this->col_insert(B, col_); }

void DenseMatrix::col_join(const DenseMatrix &B) { this->row_insert(B, row_); }
...
void DenseMatrix::col_insert(const DenseMatrix &B, unsigned pos) {
  ...
  for (unsigned i = row; i-- > 0;) {
    for (unsigned j = col; j-- > 0;) {
      if (j >= pos) {
        this->m_[i * (col + B.col_) + j + B.col_] = this->m_[i * col + j];
      } else {
...
void pivoted_gaussian_elimination(const DenseMatrix &A, DenseMatrix &B,
  ...
  for (i = 0; i < col - 1; i++) {
    ...
    for (j = i + 1; j < row; j++) {
      for (k = i + 1; k < col; k++)
        B.m_[j * col + k] =
            sub(B.m_[j * col + k], mul(B.m_[j * col + i], B.m_[i * col + k]));
      B.m_[j * col + i] = zero;
    }
...
void diagonal_solve(const DenseMatrix &A, const DenseMatrix &b,
                    DenseMatrix &x) {
  ...
  for (unsigned k = 0; k < sys; k++) {
    for (int i = col - 1; i >= 0; i--) {
      for (unsigned j = i + 1; j < col; j++)
        x.m_[i * sys + k] =
            sub(x.m_[i * sys + k], mul(U.m_[i * col + j], x.m_[j * sys + k]));
...
void forward_substitution(const DenseMatrix &A, const DenseMatrix &b,
  ...
  for (unsigned k = 0; k < b.col_; k++) {
    for (unsigned i = 0; i < col - 1; i++) {
      for (unsigned j = i + 1; j < col; j++) {
        x.m_[j * sys + k] = sub(mul(A.m_[i * col + i], x.m_[j * sys + k]),
...
void fraction_free_gaussian_elimination_solve(const DenseMatrix &A,
  ...
  for (i = 0; i < col - 1; i++)
    for (j = i + 1; j < col; j++) {
      for (k = 0; k < bcol; k++) {
        ...
        if (i > 0)
          b_.m_[j * bcol + k] =
              div(b_.m_[j * bcol + k], A_.m_[i * col - col + i - 1]);
      }
  ...
  for (k = 0; k < bcol; k++) {
    for (i = col - 1; i >= 0; i--) {
      for (j = i + 1; j < col; j++)
        b_.m_[i * bcol + k] = sub(b_.m_[i * bcol + k],
...
void fraction_free_LU(const DenseMatrix &A, DenseMatrix &LU) {
  SYMENGINE_ASSERT(A.row_ == A.col_ and LU.row_ == LU.col_ and
                   A.row_ == LU.row_);
...
void LU(const DenseMatrix &A, DenseMatrix &L, DenseMatrix &U) {
  ...
  for (j = 0; j < n; j++) {
    for (i = 0; i < j; i++)
      for (k = 0; k < i; k++)
        U.m_[i * n + j] =
            sub(U.m_[i * n + j], mul(U.m_[i * n + k], U.m_[k * n + j]));
...
void pivoted_LU(const DenseMatrix &A, DenseMatrix &LU, permutelist &pl) {
  ...
  for (j = 0; j < n; j++) {
    for (i = 0; i < j; i++)
      for (k = 0; k < i; k++)
        LU.m_[i * n + j] =
            sub(LU.m_[i * n + j], mul(LU.m_[i * n + k], LU.m_[k * n + j]));
    pivot = -1;
    for (i = j; i < n; i++) {
      for (k = 0; k < j; k++) {
        LU.m_[i * n + j] =
...
void QR(const DenseMatrix &A, DenseMatrix &Q, DenseMatrix &R) {
  ...
  SYMENGINE_ASSERT(Q.row_ == row and Q.col_ == col and R.row_ == col and
                   R.col_ == col);
...
void berkowitz(const DenseMatrix &A, std::vector<DenseMatrix> &polys) {
  ...
  for (i = 0; i < col - 1; i++) {
    ...
    for (l = 0; l < t_row; l++) {
      B.m_[l] = zero;
      for (m = 0; m < t_col; m++) {
        B.m_[l] = add(B.m_[l], mul(transforms[col - 2 - i].m_[l * t_col + m],
                                   polys[i].m_[m]));
...
void cross(const DenseMatrix &A, const DenseMatrix &B, DenseMatrix &C) {
  SYMENGINE_ASSERT((A.row_ * A.col_ == 3 and B.row_ * B.col_ == 3) and
                   (A.row_ == C.row_ and A.col_ == C.col_));
