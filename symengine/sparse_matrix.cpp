...
CSRMatrix::CSRMatrix(unsigned row, unsigned col, std::vector<unsigned> &&p,
    ...
    : p_{std::move(p)}, j_{std::move(j)}, x_{std::move(x)}, row_(row),
      col_(col) {
  SYMENGINE_ASSERT(is_canonical());
}
...
// Add a scalar
void CSRMatrix::add_scalar(const RCP<const Basic> &k,
                           MatrixBase &result) const {
  throw NotImplementedError("Not Implemented");
}

// Multiply by a scalar
void CSRMatrix::mul_scalar(const RCP<const Basic> &k,
                           MatrixBase &result) const {
  throw NotImplementedError("Not Implemented");
}
...
bool CSRMatrix::csr_has_canonical_format(const std::vector<unsigned> &p_,
  ...
  return csr_has_sorted_indices(p_, j_, row_) and
         not csr_has_duplicates(p_, j_, row_);
}
...
void csr_binop_csr_canonical(
  ...
  SYMENGINE_ASSERT(A.row_ == B.row_ and A.col_ == B.col_ and
                   C.row_ == A.row_ and C.col_ == A.col_);
