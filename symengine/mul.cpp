...
bool Mul::is_canonical(const RCP<const Number> &coef,
  ...
  for (const auto &p : dict) {
    ...
    if ((is_a<Integer>(*p.first) or is_a<Rational>(*p.first)) and
        is_a<Integer>(*p.second))
      return false;
    // e.g. 0**x
    if (is_a<Integer>(*p.first) and
        down_cast<const Integer &>(*p.first).is_zero())
      return false;
    // e.g. 1**x
    if (is_a<Integer>(*p.first) and
        down_cast<const Integer &>(*p.first).is_one())
      return false;
    // e.g. x**0
    if (is_a_Number(*p.second) and
        down_cast<const Number &>(*p.second).is_zero())
      return false;
    ...
    if (is_a<Mul>(*p.first)) {
      if (is_a<Integer>(*p.second))
        return false;
      if (neq(*down_cast<const Mul &>(*p.first).coef_, *one) and
    ...
    if (is_a_Number(*p.first) and
        not down_cast<const Number &>(*p.first).is_exact() and
        is_a_Number(*p.second) and
        not down_cast<const Number &>(*p.second).is_exact())
      return false;
  }
  return true;
}
...
bool Mul::__eq__(const Basic &o) const {
  if (is_a<Mul>(o) and eq(*coef_, *(down_cast<const Mul &>(o).coef_)) and
      unified_eq(dict_, down_cast<const Mul &>(o).dict_))
    return true;
  ...
  if (it == d.end()) {
    ...
    if (is_a_Number(*it->second) and is_a_Number(*exp)) {
      ...
      if (is_a_Number(*it->second) and
          down_cast<const Number &>(*(it->second)).is_zero()) {
        d.erase(it);
      }
    }
  }
  ...
  if (it == d.end()) {
    ...
    if (is_a<Integer>(*it->second)) {
      ...
      if (is_a<Integer>(*t) or is_a<Rational>(*t)) {
        ...
        if (down_cast<const Integer &>(*(it->second)).is_one()) {
          imulnum(outArg(*coef), rcp_static_cast<const Number>(t));
          d.erase(it);
        } else if (down_cast<const Integer &>(*(it->second)).is_minus_one()) {
          idivnum(outArg(*coef), rcp_static_cast<const Number>(t));
          d.erase(it);
        }
        return;
    ...
    if (is_a_Number(*it->second)) {
      if (down_cast<const Number &>(*it->second).is_zero()) {
        ...
        if (is_a<Integer>(*it->second) or
            (neq(*m->coef_, *one) and neq(*m->coef_, *minus_one))) {
