...
class ExpressionParser {
  ...
  RCP<const Basic> parse_string(unsigned int l, unsigned int h) {
    ...
    for (unsigned int iter = l; iter < h; ++iter) {
      if (is_single_character_operator(iter) or
          (iter + 1 < h and is_dual_character_operator(iter + 1))) {

        if (s[iter] == '+' or s[iter] == '-') {
          if (iter > l + 1 and iter < h - 1 and s[iter - 1] == 'e' and
              s[iter - 2] >= '0' and s[iter - 2] <= '9' and
              s[iter + 1] >= '0' and s[iter + 1] <= '9') {
            continue;
          }
        }
        ...
        if (s[iter] == '+') {
        ...
        } else if (s[iter] == '*' and iter + 1 < h and s[iter + 1] == '*') {
          result = pow(result, parse_string(iter + 2, operator_end[iter]));
          iter = operator_end[iter] - 1;
          ...
          result = Eq(result, parse_string(iter + 2, operator_end[iter]));
          iter = operator_end[iter] - 1;

        } else if (s[iter] == '<' and iter + 1 < h and s[iter + 1] == '=') {
          result = Le(result, parse_string(iter + 2, operator_end[iter]));
          iter = operator_end[iter] - 1;

        } else if (s[iter] == '>' and iter + 1 < h and s[iter + 1] == '=') {
          result = Ge(result, parse_string(iter + 2, operator_end[iter]));
          iter = operator_end[iter] - 1;

        } else if (s[iter] == '<') {
          result = Lt(result, parse_string(iter + 1, operator_end[iter]));
          iter = operator_end[iter] - 1;

        } else if (s[iter] == '>') {
          result = Gt(result, parse_string(iter + 1, operator_end[iter]));
          iter = operator_end[iter] - 1;
  ...
  RCP<const Basic> set_result(const std::string &expr) {
    ...
    if (has_numeric_part) {
      ...
      if (lendptr == endptr) {
        ...
        if (digits <= 15) {
          ...
          long prec =
              std::max(long(1), std::lround((digits + 1) * 3.3219280948873626));
    ...
    if (c != constants.end()) {
      ...
      for (unsigned i = 0; i < lexpr.length(); ++i) {
        if (not std::isalnum(lexpr[i]) and lexpr[i] != '_' and lexpr[i] >= 0) {
...
public:
  ...
  RCP<const Basic> parse_expr(const std::string &in) {
    ...
    for (int i = s_len - 1; i >= 0; i--) {
      if (is_single_character_operator(i) or is_dual_character_operator(i)) {
        std::string x;
        x = s[i];
        ...
        if (x == "(") {
          ...
          if (x == "+" or x == "-") {
            if (i > 1 and i < (int)s_len - 1 and s[i - 1] == 'e' and
                s[i - 2] >= '0' and s[i - 2] <= '9' and s[i + 1] >= '0' and
                s[i + 1] <= '9') {
              // numeric like 1e-10
          ...
          if (last_char_was_op and (last_char == "-" or last_char == "+"))
            op_stack.pop();
