...
bool order(const DenseMatrix &t, const std::vector<DenseMatrix> &basis,
  ...
  for (unsigned j = 0; j < t.ncols(); j++) {
    ...
    integer_class b_ =
        down_cast<const Integer &>(*basis[k].get(0, j)).as_integer_class();
  ...
  while (P.size() > 0) {
    ...
    if (product.eq(col_zero) and not t.eq(row_zero)) {
      ...
      for (unsigned i = 0; i < q; i++) {
        ...
        T.set(0, i, down_cast<const Integer &>(*T.get(0, i)).addint(*one));

        if (i > 0) {
          SYMENGINE_ASSERT(is_a<Integer>(*T.get(0, i - 1)));
          T.set(0, i - 1,
                down_cast<const Integer &>(*T.get(0, i - 1)).subint(*one));
        }
        ...
        if (F[i] == false and
            ((dot->is_negative() and
              is_minimum(T, basis, numeric_cast<unsigned>(basis.size()))) or
             t.eq(row_zero))) {
          P.push_back(T);
          n = n + 1;
