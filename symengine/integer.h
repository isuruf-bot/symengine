...
public:
  ...
  Integer(const integer_class &_i) : i(_i) { SYMENGINE_ASSIGN_TYPEID() }
  Integer(integer_class &&_i) : i(std::move(_i)) { SYMENGINE_ASSIGN_TYPEID() }
  ...
  inline const integer_class &as_integer_class() const { return this->i; }
  //! \return `true` if `0`
  ...
  inline RCP<const Number> powint(const Integer &other) const {
    if (not(mp_fits_ulong_p(other.i))) {
      if (other.i > 0u)
        throw SymEngineException("powint: 'exp' does not fit unsigned long.");
      else
  ...
  inline RCP<const Integer> neg() const { return make_rcp<const Integer>(-i); }
