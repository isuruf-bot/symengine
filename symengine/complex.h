...
public:
  ...
  //! \returns `true`
  inline virtual bool is_complex() const { return true; }
  ...
  inline RCP<const Number> addcomp(const Rational &other) const {
    return from_mpq(this->real_ + other.as_rational_class(), this->imaginary_);
  }
  /*! Add Complex
   * \param other of type Integer
   * */
  inline RCP<const Number> addcomp(const Integer &other) const {
    return from_mpq(this->real_ + other.as_integer_class(), this->imaginary_);
  }
  ...
  inline RCP<const Number> rsubcomp(const Rational &other) const {
    return from_mpq(other.as_rational_class() - this->real_, -this->imaginary_);
  }
  /*! Subtract Complex from other
   * \param other of type Integer
   * */
  inline RCP<const Number> rsubcomp(const Integer &other) const {
    return from_mpq(other.as_integer_class() - this->real_, -this->imaginary_);
  }
  ...
  inline RCP<const Number> rdivcomp(const Integer &other) const {
    ...
    if (get_num(modulus_sq_this) == 0) {
      ...
      return from_mpq(
          (this->real_ * other.as_integer_class()) / modulus_sq_this,
          (this->imaginary_ * (-other.as_integer_class())) / modulus_sq_this);
    }
  }
  /*! Pow Complex
   * \param other of type Integer
