...
public:
  ...
  void bvisit(const Pow &x) {
    ...
    if (is_a<Integer>(*exp)) {
      ...
      p = Series::series_exp(
          Poly(apply(exp) * Series::series_log(apply(base), var, prec)), var,
          prec);
    }
  }
...
public:
  template <typename T,
            typename = enable_if_t<std::is_base_of<TrigBase, T>::value or
                                   std::is_base_of<HyperbolicBase, T>::value>>
  ...
  void bvisit(const Pow &pow) {
    ...
    map_basic_basic subsx0{{x_, integer(0)}};
    // exp(const) or x^-1
    if ((base->__eq__(*E) and exp->subs(subsx0)->__neq__(*integer(0))) or
        (is_a_Number(*exp) and down_cast<const Number &>(*exp).is_negative() and
         base->subs(subsx0)->__eq__(*integer(0)))) {
      needs_ = true;
      stop_ = true;
    }
  }
