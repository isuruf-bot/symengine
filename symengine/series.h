...
public:
  ...
  inline virtual unsigned get_degree() const { return degree_; }
  ...
  inline virtual bool is_complex() const { return false; }

  inline virtual bool __eq__(const Basic &o) const {
    return (is_a<Series>(o) and var_ == down_cast<const Series &>(o).var_ and
            p_ == down_cast<const Series &>(o).p_ and
            degree_ == down_cast<const Series &>(o).degree_);
  }
  ...
  virtual RCP<const Number> pow(const Number &other) const {
    ...
    if (is_a<Series>(other)) {
      ...
      p = Series::pow(
          p_, numeric_cast<int>((down_cast<const Integer &>(other).as_int())),
          deg);
      return make_rcp<Series>(p, var_, deg);
    } else if (other.get_type_code() < Series::type_code_id) {
  ...
  static inline Poly series_invert(const Poly &s, const Poly &var,
                                   unsigned int prec) {
    if (s == 0)
      throw DivisionByZeroError("Series::series_invert: Division By Zero");
    if (s == 1)
      return Poly(1);
  ...
  static inline Poly series_exp(const Poly &s, const Poly &var,
    ...
    for (const auto step : steps) {
      res_p =
          Series::mul(res_p, t - Series::series_log(res_p, var, step), step);
    }
    if (c != 0) {
  ...
  static inline Poly series_lambertw(const Poly &s, const Poly &var,
    ...
    for (const auto step : steps) {
      ...
      const Poly p3(
          Series::series_invert(Series::mul(e, (p1 + 1), step), var, step));
  ...
  static inline Poly series_asinh(const Poly &s, const Poly &var,
    ...
    const Poly p(Series::series_nthroot(Series::pow(s, 2, prec - 1) + 1, 2, var,
                                        prec - 1));
