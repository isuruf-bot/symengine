...
public:
  struct DictLess {
    bool operator()(const GaloisFieldDict &a, const GaloisFieldDict &b) const {
  ...
  GaloisFieldDict &operator%=(const GaloisFieldDict &other) {
    ...
    for (auto it = deg_dividend + 1; it-- != 0;) {
      coeff = dict_out[it];
      auto lb =
          deg_divisor + it > deg_dividend ? deg_divisor + it - deg_dividend : 0;
  ...
  const std::vector<integer_class> &get_dict() const { return dict_; }
...
public:
  ...
  iterator begin() const { return get_poly().dict_.begin(); }
  iterator end() const { return get_poly().dict_.end(); }
  reverse_iterator obegin() const { return get_poly().dict_.rbegin(); }
  reverse_iterator oend() const { return get_poly().dict_.rend(); }
