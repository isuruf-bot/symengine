...
const char *symengine_version() { return SYMENGINE_VERSION; }
...
void basic_const_Catalan(basic s) { s->m = SymEngine::Catalan; }

void basic_const_GoldenRatio(basic s) { s->m = SymEngine::GoldenRatio; }

void basic_const_infinity(basic s) { s->m = SymEngine::Inf; }

void basic_const_neginfinity(basic s) { s->m = SymEngine::NegInf; }

void basic_const_complex_infinity(basic s) { s->m = SymEngine::ComplexInf; }

void basic_const_nan(basic s) { s->m = SymEngine::Nan; }
...
CWRAPPER_OUTPUT_TYPE integer_get_mpz(mpz_t a, const basic s) {
  ...
  SYMENGINE_ASSERT(is_a<Integer>(*(s->m)));
  mpz_set(a,
          get_mpz_t((down_cast<const Integer &>(*(s->m))).as_integer_class()));
  CWRAPPER_END
}
#endif
...
CWRAPPER_OUTPUT_TYPE rational_set_ui(basic s, unsigned long a,
                                     unsigned long b) {
  CWRAPPER_BEGIN
  s->m = SymEngine::Rational::from_mpq(rational_class(a, b));
  CWRAPPER_END
...
CWRAPPER_OUTPUT_TYPE rational_get_mpq(mpq_t a, const basic s) {
  ...
  mpq_set(
      a, get_mpq_t((down_cast<const Rational &>(*(s->m))).as_rational_class()));
  CWRAPPER_END
}
...
dcomplex complex_double_get(const basic s) {
  ...
  dcomplex d;
  d.real =
      (down_cast<const ComplexDouble &>(*(s->m)).as_complex_double()).real();
  d.imag =
      (down_cast<const ComplexDouble &>(*(s->m)).as_complex_double()).imag();
  return d;
}
...
int is_a_Number(const basic s) { return (int)is_a_Number(*(s->m)); }
...
int is_a_RealDouble(const basic c) { return is_a<RealDouble>(*(c->m)); }
int is_a_ComplexDouble(const basic c) { return is_a<ComplexDouble>(*(c->m)); }
...
CDenseMatrix *dense_matrix_new() { return new CDenseMatrix(); }
...
CSparseMatrix *sparse_matrix_new() { return new CSparseMatrix; }

void dense_matrix_free(CDenseMatrix *self) { delete self; }

void sparse_matrix_free(CSparseMatrix *self) { delete self; }

void sparse_matrix_init(CSparseMatrix *s) { s->m = SymEngine::CSRMatrix(); }
...
CWRAPPER_OUTPUT_TYPE dense_matrix_inv(CDenseMatrix *s,
...
dense_matrix_submatrix(CDenseMatrix *s, const CDenseMatrix *mat,
  ...
  mat->m.submatrix(s->m, numeric_cast<unsigned>(r1), numeric_cast<unsigned>(c1),
                   numeric_cast<unsigned>(r2), numeric_cast<unsigned>(c2),
                   numeric_cast<unsigned>(r), numeric_cast<unsigned>(c));
  CWRAPPER_END
}
...
int is_a_DenseMatrix(const CDenseMatrix *c) { return is_a<DenseMatrix>(c->m); }

int is_a_SparseMatrix(const CSparseMatrix *c) { return is_a<CSRMatrix>(c->m); }
...
CWRAPPER_OUTPUT_TYPE ntheory_gcd_ext(basic g, basic s, basic t, const basic a,
  ...
  SymEngine::gcd_ext(SymEngine::outArg(g_), SymEngine::outArg(s_),
                     SymEngine::outArg(t_), down_cast<const Integer &>(*(a->m)),
                     down_cast<const Integer &>(*(b->m)));
  g->m = g_;
  s->m = s_;
  t->m = t_;
