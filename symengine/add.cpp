...
bool Add::is_canonical(const RCP<const Number> &coef,
  ...
  for (const auto &p : dict) {
    ...
    if (is_a<Mul>(*p.first) and
        not(down_cast<const Mul &>(*p.first).get_coef()->is_one()))
      return false;
  }
  return true;
}
...
bool Add::__eq__(const Basic &o) const {
  if (is_a<Add>(o) and eq(*coef_, *(down_cast<const Add &>(o).coef_)) and
      unified_eq(dict_, down_cast<const Add &>(o).dict_))
    return true;
    ...
    if (is_a<Integer>(*(p->second))) {
      ...
      if (is_a<Mul>(*(p->first))) {
...
#else
        {
          ...
          map_basic_basic d2 = down_cast<const Mul &>(*(p->first)).get_dict();
