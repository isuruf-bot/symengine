...
bool Interval::__eq__(const Basic &o) const {
  if (is_a<Interval>(o)) {
    ...
    return ((this->left_open_ == s.left_open_) and
            (this->right_open_ == s.right_open_) and
...
RCP<const Set> Interval::set_intersection(const RCP<const Set> &o) const {
  ...
  if (is_a<UniversalSet>(*o) or is_a<EmptySet>(*o) or is_a<FiniteSet>(*o) or
      is_a<Union>(*o)) {
...
RCP<const Set> Interval::set_complement(const RCP<const Set> &o) const {
  if (is_a<Interval>(*o)) {
    ...
    if (eq(*max({start_, other.start_}), *start_)) {
      cont.insert(interval(other.get_start(), start_, other.get_left_open(),
                           not left_open_));
    }
...
RCP<const Set> FiniteSet::set_union(const RCP<const Set> &o) const {
  ...
  if (is_a<Interval>(*o)) {
    ...
    if (not container.empty()) {
      if (left == other.get_left_open() and right == other.get_right_open()) {
        return SymEngine::make_set_union({finiteset(container), o});
      } else {
        return SymEngine::make_set_union(set_set(
            {finiteset(container),
      ...
      if (left == other.get_left_open() and right == other.get_right_open()) {
        ...
        return interval(other.get_start(), other.get_end(), left, right);
      }
    }
  }
...
RCP<const Set> FiniteSet::set_complement(const RCP<const Set> &o) const {
  ...
  if (is_a<Interval>(*o)) {
    ...
    set_basic rest;
    bool left_open = other.get_left_open(), right_open = other.get_right_open();
    ...
    if (eq(*max({last, other.get_end()}), *other.get_end())) {
      intervals.insert(interval(last, other.get_end(), left_open, right_open));
    }
    if (rest.empty()) {
      return SymEngine::make_set_union(intervals);
    } else {
      return make_rcp<const Complement>(SymEngine::make_set_union(intervals),
                                        finiteset(rest));
    }
  }
...
bool Complement::__eq__(const Basic &o) const {
  if (is_a<Complement>(o)) {
    ...
    return unified_eq(universe_, other.universe_) and
           unified_eq(container_, other.container_);
  }
  return false;
}
...
bool ConditionSet::is_canonical(const RCP<const Basic> &sym,
                                const RCP<const Boolean> &condition) {
  if (eq(*condition, *boolFalse) or eq(*condition, *boolTrue) or
      not is_a_sub<Symbol>(*sym)) {
    return false;
...
bool ConditionSet::__eq__(const Basic &o) const {
  if (is_a<ConditionSet>(o)) {
    ...
    return unified_eq(sym, other.get_symbol()) and
           unified_eq(condition_, other.get_condition());
  }
  return false;
}
...
bool ImageSet::__eq__(const Basic &o) const {
  if (is_a<ImageSet>(o)) {
    ...
    return unified_eq(sym_, other.sym_) and unified_eq(expr_, other.expr_) and
           unified_eq(base_, other.base_);
  }
  return false;
}
...
bool ImageSet::is_canonical(const RCP<const Basic> &sym,
                            ...
                            const RCP<const Set> &base) {
  if (not is_a_sub<Symbol>(*sym) or eq(*expr, *sym) or is_a_Number(*expr) or
      eq(*base, *emptyset()))
    return false;
  return true;
}
...
RCP<const Set> set_intersection(const set_set &in) {
  ...
  for (auto it = incopy.begin(); it != incopy.end(); ++it) {
    if (is_a<Complement>(**it)) {
      auto container = down_cast<const Complement &>(**it).get_container();
  ...
  if (is_a<And>(*condition)) {
    ...
    for (auto it = cont.begin(); it != cont.end(); it++) {
      if (is_a<Contains>(**it) and
          eq(*down_cast<const Contains &>(**it).get_expr(), *sym) and
          is_a<FiniteSet>(*down_cast<const Contains &>(**it).get_set())) {
        auto fset = down_cast<const Contains &>(**it).get_set();
        auto fcont = down_cast<const FiniteSet &>(*fset).get_container();
