...
int _factor_trial_division_sieve(integer_class &factor,
...
int factor(const Ptr<RCP<const Integer>> &f, const Integer &n, double B1) {
  ...
  if (mp_perfect_power_p(_n)) {
    ...
    if (mp_probab_prime_p(_n, 25) > 0) { // most probably, n is a prime
      ...
      mp_demote(_f);
      if (not ret_val)
        throw SymEngineException("ECM failed to factor the given number");
    }
  }
#else
...
void prime_factors(std::vector<RCP<const Integer>> &prime_list,
  ...
  if (not mp_fits_ulong_p(sqrtN) or
...
void prime_factor_multiplicities(map_integer_uint &primes_mul,
                                 const Integer &n) {
  integer_class sqrtN;
  integer_class _n = n.as_integer_class();
  unsigned count;
...
void Sieve::_extend(unsigned limit) {
  ...
  for (; start <= limit; start += 2 * segment) {
    ...
    for (unsigned index = 1;
         index < _primes.size() and _primes[index] * _primes[index] <= finish;
         ++index) {
...
RCP<const Integer> carmichael(const RCP<const Integer> &n) {
  ...
  for (const auto it : prime_mul) {
    ...
    if (p == 2 and
        multiplicity > 2) { // For powers of 2 greater than 4 divide by 2.
      multiplicity--;
    }
    t = p - 1;
...
bool _nthroot_mod_prime_power(std::vector<RCP<const Integer>> &roots,
  ...
  if (a % p != 0) {
    ...
    if (_a == 0) {
      ...
      if (r < n or r % n != 0 or
          not _nthroot_mod_prime_power(_roots, _a, n, p, k - r, all_roots)) {
        return false;
      }
...
bool _is_nthroot_mod_prime_power(const integer_class &a, const integer_class &n,
  ...
  if (a % p != 0) {
    ...
    if (_a == 0) {
      ...
      if (r < n or r % n != 0 or
          not _is_nthroot_mod_prime_power(_a, n, p, k - r)) {
        return false;
      }
      return true;
    }
...
void powermod_list(std::vector<RCP<const Integer>> &pows,
                   ...
                   const RCP<const Integer> &m) {
  if (is_a<Integer>(*b)) {
    integer_class t = mp_abs(down_cast<const Integer &>(*b).as_integer_class());
    mp_powm(t, a->as_integer_class(), t, m->as_integer_class());
    if (b->is_negative()) {
      bool ret_val = mp_invert(t, t, m->as_integer_class());
      if (not ret_val)
...
*/
{
  ...
  for (const auto &it : prime_mul) {
    ret_val =
        _is_nthroot_mod_prime_power(a.as_integer_class(), n.as_integer_class(),
                                    it.first->as_integer_class(), it.second);
    if (not ret_val)
      return false;
  }
