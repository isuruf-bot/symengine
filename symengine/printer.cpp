...
std::string print_double(double d) {
  ...
  s << d;
  auto str_ = s.str();
  if (str_.find(".") == std::string::npos and
      str_.find("e") == std::string::npos) {
...
void StrPrinter::bvisit(const RealDouble &x) { str_ = print_double(x.i); }
...
void StrPrinter::bvisit(const EmptySet &x) { str_ = "EmptySet"; }
...
void StrPrinter::bvisit(const UniversalSet &x) { str_ = "UniversalSet"; }
...
void StrPrinter::bvisit(const RealMPFR &x) {
  ...
  long digits =
      std::max(long(1), std::lround(static_cast<double>(x.i.get_prec()) /
                                    3.3219280948873626) -
                            1);
  char *c = mpfr_get_str(nullptr, &ex, 10, digits, x.i.get_mpfr_t(), MPFR_RNDN);
  std::ostringstream s;
  str_ = std::string(c);
  if (str_.at(0) == '-') {
    s << '-';
...
void StrPrinter::bvisit(const Mul &x) {
  ...
  for (const auto &p : x.get_dict()) {
    if ((is_a<Integer>(*p.second) or is_a<Rational>(*p.second)) and
        down_cast<const Number &>(*p.second).is_negative() and
        neq(*(p.first), *E)) {
...
template <typename T> char _print_sign(const T &i) {
  if (i < 0) {
    return '-';
  } else {
    return '+';
...
template <typename P> std::string upoly_print(const P &x) {
  std::ostringstream s;
  // bool variable needed to take care of cases like -5, -x, -3*x etc.
  bool first = true;
  // we iterate over the map in reverse order so that highest degree gets
...
void StrPrinter::bvisit(const UIntPoly &x) { str_ = upoly_print<UIntPoly>(x); }

void StrPrinter::bvisit(const URatPoly &x) { str_ = upoly_print<URatPoly>(x); }
