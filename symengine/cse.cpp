  ...
  get_subset_candidates(const Container1 &argset,
    ...
    for (const auto &arg : argset) {
      std::set_intersection(
          indices.begin(), indices.end(), arg_to_funcset[arg].begin(),
          arg_to_funcset[arg].end(), std::back_inserter(intersect_result));
  ...
  for (unsigned i = 0; i < funcs.size(); i++) {
    ...
    while (common_arg_candidates.size() > 0) {
      ...
      if (diff_i.size() > 0) {
        ...
        // unevaluated version.
        com_func_number = arg_tracker.get_or_add_value_number(funcs[i].first);
      }
      ...
      for (unsigned k :
           arg_tracker.get_subset_candidates(com_args, common_arg_candidates)) {
...
public:
  ...
  void bvisit(const Derivative &x) { return; }
  void bvisit(const Subs &x) { return; }
  ...
  void bvisit(const Pow &x) {
    if (not is_seen(x)) {
      ...
      if (is_a_Number(*ex) and static_cast<const Number &>(*ex).is_negative()) {
    ...
    if (not is_seen(x)) {
      ...
      if (x.get_coef()->is_negative()) {
        ...
        if (not is_a<Symbol>(*neg_expr)) {
          opt_subs[expr] = function_symbol("mul", {integer(-1), neg_expr});
...
void tree_cse(vec_pair &replacements, vec_basic &reduced_exprs,
  ...
  RebuildVisitor rebuild_visitor(subs, opt_subs, to_eliminate, excluded_symbols,
                                 replacements);
