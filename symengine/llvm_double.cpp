...
void LLVMDoubleVisitor::init(const vec_basic &inputs, const vec_basic &outputs,
  ...
  for (int i = 0; i < 2; i++) {
    inp.push_back(llvm::PointerType::get(llvm::Type::getDoubleTy(*context), 0));
  }
  ...
  for (unsigned i = 0; i < inputs.size(); i++) {
    ...
    auto index = llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context), i);
    auto ptr =
        builder->CreateGEP(llvm::Type::getDoubleTy(*context), input_arg, index);
...
void LLVMDoubleVisitor::bvisit(const Integer &x, bool as_int32) {
  if (as_int32) {
    ...
    result_ = llvm::ConstantInt::get(llvm::Type::getInt32Ty(mod->getContext()),
                                     d, true);
  } else {
    result_ = llvm::ConstantFP::get(llvm::Type::getDoubleTy(mod->getContext()),
...
void LLVMDoubleVisitor::bvisit(const RealDouble &x) { set_double(x.i); }
...
llvm::Function *LLVMDoubleVisitor::get_powi() {
  ...
  return llvm::Intrinsic::getDeclaration(mod, llvm::Intrinsic::powi, arg_type);
}
...
LLVMDoubleVisitor::get_external_function(const std::string &name) {
  ...
  llvm::AttributeSet func_attr_set;
  {
    ...
    llvm::AttributeSet attr_set;
    {
      ...
      attr_set = llvm::AttributeSet::get(mod->getContext(), ~0U, attr_builder);
    }
