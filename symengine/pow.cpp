...
bool Pow::is_canonical(const Basic &base, const Basic &exp) const {
  ...
  if ((is_a<Rational>(base) or is_a<Integer>(base)) and is_a<Rational>(exp) and
      (down_cast<const Rational &>(exp).as_rational_class() < 0 or
       down_cast<const Rational &>(exp).as_rational_class() > 1))
    return false;
  // Purely Imaginary complex numbers with integral powers are expanded
  // e.g (2I)**3
  if (is_a<Complex>(base) and down_cast<const Complex &>(base).is_re_zero() and
      is_a<Integer>(exp))
    return false;
  // e.g. 0.5^2.0 should be represented as 0.25
  if (is_a_Number(base) and not down_cast<const Number &>(base).is_exact() and
      is_a_Number(exp) and not down_cast<const Number &>(exp).is_exact())
    return false;
  return true;
}
...
bool Pow::__eq__(const Basic &o) const {
  if (is_a<Pow>(o) and eq(*base_, *(down_cast<const Pow &>(o).base_)) and
...
RCP<const Basic> pow(const RCP<const Basic> &a, const RCP<const Basic> &b) {
  ...
  if (eq(*a, *zero)) {
    if (is_a_Number(*b) and rcp_static_cast<const Number>(b)->is_positive()) {
      return zero;
    } else if (is_a_Number(*b) and
               rcp_static_cast<const Number>(b)->is_negative()) {
      return ComplexInf;
    } else {
  ...
  if (is_a_Number(*b)) {
    if (is_a_Number(*a)) {
      ...
      } else if (is_a<Complex>(*b) and
                 down_cast<const Number &>(*a).is_exact()) {
        return make_rcp<const Pow>(a, b);
      } else {
        return down_cast<const Number &>(*a)
            .pow(*rcp_static_cast<const Number>(b));
