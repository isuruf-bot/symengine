...
public:
  ...
  void mul_expand_two(const RCP<const Basic> &a, const RCP<const Basic> &b) {
    ...
    if (is_a<Add>(*a) && is_a<Add>(*b)) {
      iaddnum(
          outArg(coeff),
          _mulnum(multiply, _mulnum(down_cast<const Add &>(*a).get_coef(),
...
// Improves (x+1)**3*(x+2)**3*...(x+350)**3 expansion from 0.97s to 0.93s:
      ...
      for (auto &p : (down_cast<const Add &>(*a)).get_dict()) {
        ...
        for (auto &q : (down_cast<const Add &>(*b)).get_dict()) {
          ...
          if (is_a_Number(*term)) {
            ...
            if (is_a<Mul>(*term) &&
              ...
              map_basic_basic d2 = down_cast<const Mul &>(*term).get_dict();
              term = Mul::from_dict(one, std::move(d2));
              Add::dict_add_term(d_, _mulnum(_mulnum(temp, q.second), coef2),
                                 term);
            } else {
        ...
        Add::dict_add_term(
            d_, _mulnum(down_cast<const Add &>(*b).get_coef(), temp), p.first);
      }
...
#if defined(HAVE_SYMENGINE_RESERVE)
      d_.reserve(d_.size() + (down_cast<const Add &>(*b)).get_dict().size());
#endif
      for (auto &q : (down_cast<const Add &>(*b)).get_dict()) {
        ...
        if (is_a_Number(*term)) {
          ...
          if (is_a<Mul>(*term) &&
              !(down_cast<const Mul &>(*term).get_coef()->is_one())) {
            // Tidy up things like {2x: 3} -> {x: 6}
            RCP<const Number> coef2 = down_cast<const Mul &>(*term).get_coef();
            // We make a copy of the dict_:
            map_basic_basic d2 = down_cast<const Mul &>(*term).get_dict();
            term = Mul::from_dict(one, std::move(d2));
            Add::dict_add_term(d_, _mulnum(_mulnum(q.second, a_coef), coef2),
                               term);
          } else {
      ...
      if (eq(*a_term, *one)) {
        ...
        Add::dict_add_term(
            d_, _mulnum(down_cast<const Add &>(*b).get_coef(), a_coef), a_term);
      }
      return;
    }
    ...
    for (auto p = base_dict.begin(); p != base_dict.end(); ++p) {
      for (auto q = p; q != base_dict.end(); ++q) {
        if (q == p) {
          ...
          _coef_dict_add_term(
              _mulnum(multiply,
                      _mulnum((*p).second, _mulnum((*q).second, two))),
              mul((*q).first, (*p).first));
        }
      }
    }
    ...
    for (auto &p : r) {
      ...
      for (; power != p.first.end(); ++power, ++i2) {
        if (*power > 0) {
          ...
          if (is_a<Integer>(*base)) {
            _imulnum(outArg(overall_coeff),
                     rcp_static_cast<const Number>(
                         down_cast<const Integer &>(*base).powint(*exp)));
            ...
            tmp = pow(base, exp);
            if (is_a<Mul>(*tmp)) {
              for (auto &p : (down_cast<const Mul &>(*tmp)).get_dict()) {
                Mul::dict_add_term_new(outArg(overall_coeff), d, p.second,
                                       p.first);
              }
              _imulnum(outArg(overall_coeff),
          ...
          if (!(i2->second->is_one())) {
            _imulnum(outArg(overall_coeff),
                     pownum(i2->second, rcp_static_cast<const Number>(exp)));
          }
        }
      }
      ...
      if (is_a_Number(*term)) {
        iaddnum(outArg(coeff),
                _mulnum(_mulnum(multiply, rcp_static_cast<const Number>(term)),
                        coef2));
      } else {
        if (is_a<Mul>(*term) &&
            !(down_cast<const Mul &>(*term).get_coef()->is_one())) {
          // Tidy up things like {2x: 3} -> {x: 6}
          _imulnum(outArg(coef2), down_cast<const Mul &>(*term).get_coef());
          // We make a copy of the dict_:
          map_basic_basic d2 = down_cast<const Mul &>(*term).get_dict();
          term = Mul::from_dict(one, std::move(d2));
        }
