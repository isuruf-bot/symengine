...
public:
  ...
  ~fmpq_wrapper() SYMENGINE_NOEXCEPT { fmpq_clear(mp); }
...
public:
  ...
  fmpz_poly_factor_wrapper() { fmpz_poly_factor_init(fac); }
  ...
  ~fmpz_poly_factor_wrapper() { fmpz_poly_factor_clear(fac); }

  const fmpz_poly_factor_t &get_fmpz_poly_factor_t() const { return fac; }
  fmpz_poly_factor_t &get_fmpz_poly_factor_t() { return fac; }
};
...
public:
  ...
  fmpz_poly_wrapper() { fmpz_poly_init(poly); }
  ...
  ~fmpz_poly_wrapper() { fmpz_poly_clear(poly); }
  const fmpz_poly_t *get_fmpz_poly_t() const { return &poly; }
  fmpz_poly_t *get_fmpz_poly_t() { return &poly; }
  bool operator==(const fmpz_poly_wrapper &other) const {
    return fmpz_poly_equal(poly, *other.get_fmpz_poly_t()) == 1;
  }
  long degree() const { return fmpz_poly_degree(poly); }
  long length() const { return fmpz_poly_length(poly); }
  std::string to_string() const { return fmpz_poly_get_str(poly); }
...
public:
  ...
  long length() const { return fmpq_poly_length(poly); }
