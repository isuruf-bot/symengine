...
inline void mp_set_d(piranha::integer &i, double a) { i = a; }
...
inline auto get_mpq_t(const piranha::rational &i)
    -> decltype(i.get_mpq_view()) {
...
inline void mp_pow_ui(fmpz_wrapper &res, const fmpz_wrapper &i,
                      unsigned long n) {
  fmpz_pow_ui(res.get_fmpz_t(), i.get_fmpz_t(), n);
}

inline void mp_pow_ui(fmpq_wrapper &res, const fmpq_wrapper &i,
                      unsigned long n) {
...
inline int mp_sign(const integer_class &i) { return boost::math::sign(i); }

inline double mp_get_d(const integer_class &i) {
  return i.convert_to<double>();
}

inline void mp_set_d(integer_class &i, double a) { i.assign(a); }

inline unsigned long mp_get_ui(const integer_class &i) {
  return mp_abs(i).convert_to<unsigned long>();
}

inline long mp_get_si(const integer_class &i) { return i.convert_to<long>(); }
...
inline int mp_sign(const rational_class &i) { return i.sign(); }
...
public:
  ...
  void seed(const uint32_t &i) { _twister.seed(i); }
...
inline bool mp_root(integer_class &res, const integer_class &i,
                    unsigned long n) {
  auto _res = get_mpz_t(res);
...
public:
  ...
  void seed(const integer_class &i) { gmp_randseed(_state, get_mpz_t(i)); }
  ...
  ~mp_randstate() { gmp_randclear(_state); }
...
namespace boost {
namespace detail {
template <> struct make_unsigned_imp<SymEngine::integer_class> {
