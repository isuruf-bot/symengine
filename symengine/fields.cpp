    ...
    for (unsigned i = 0; i < get_poly().dict_.size(); i++) {
      ...
      if (i == 0) {
        ...
        if (get_poly().dict_[i] == 1) {
          ...
          args.push_back(
              Mul::from_dict(integer(get_poly().dict_[i]), {{get_var(), one}}));
        }
      } else {
        if (get_poly().dict_[i] == 1) {
...
GaloisFieldDict GaloisFieldDict::gf_sqr() const { return (*this * *this); }
...
GaloisFieldDict::gf_frobenius_monomial_base() const {
  auto n = degree();
