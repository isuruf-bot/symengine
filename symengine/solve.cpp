  ...
  if (eq(*d, *zero)) {
    ...
    if (eq(*delta, *zero)) {
      if (eq(*delta0, *zero)) {
        ...
        root3 = div(sub(mul({i4, b, c}), add(mul(d, i9), pow(b, i3))), delta0);
      }
    } else {
      ...
      root2 =
          neg(div(add(b, add(mul(cbrt1, C), div(delta0, mul(cbrt1, C)))), i3));
      root3 =
          neg(div(add(b, add(mul(cbrt2, C), div(delta0, mul(cbrt2, C)))), i3));
    }
  }
...
RCP<const Set> solve_poly_quartic(const vec_basic &coeffs,
  ...
  if (eq(*d, *zero)) {
    ...
    newcoeffs[0] = c, newcoeffs[1] = b, newcoeffs[2] = a, newcoeffs[3] = one;
...
RCP<const Set> solve_poly_heuristics(const vec_basic &coeffs,
  ...
  default:
    throw SymEngineException("expected a polynomial of order between 0 to 4");
  }
}
...
RCP<const Set> solve_poly(const RCP<const Basic> &f,
  ...
  try {
    ...
    for (const auto &elem : fac) {
      ...
      if (degree <= 4) {
        solns.insert(solve_poly_heuristics(extract_coeffs(uip), domain));
      } else {
        solns.insert(
...
public:
  ...
  bool apply(const RCP<const Basic> &b) { return apply(*b); }

  void bvisit(const Basic &x) { local_stop_ = false; }
  ...
  template <typename T, typename = enable_if_t<
                            std::is_base_of<TrigFunction, T>::value or
                            std::is_base_of<HyperbolicFunction, T>::value>>
...
public:
  ...
  void bvisit(const Basic &x) { result_ = gY_; }
  ...
  void bvisit(const Mul &x) {
    ...
    if (not eq(*indepX, *one)) {
      if (eq(*indepX, *NegInf) or eq(*indepX, *Inf) or
  ...
  void bvisit(const Pow &x) {
    if (eq(*x.get_base(), *E) and is_a<FiniteSet>(*gY_)) {
      ...
      for (const auto &elem :
        ...
        inv.insert(
            imageset(nD_, add(mul(add(mul({integer(2), nD_, pi}), logarg), I),
...
RCP<const Set> solve_trig(const RCP<const Basic> &f,
  ...
  if (eq(*soln, *emptyset()))
    ...
    set_set res;
    auto nD = dummy("n"); // use the same dummy for finding every solution set.
    auto n = symbol("n"); // replaces the above dummy in final set of solutions.
    map_basic_basic d;
    d[nD] = n;
    for (const auto &elem :
         down_cast<const FiniteSet &>(*soln).get_container()) {
      res.insert(invertComplex(exp(mul(I, sym)), finiteset({elem}), sym, nD));
    }
