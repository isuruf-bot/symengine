...
#define DIFF0(CLASS)                                                           \
  ...
  diff(const T &self, const RCP<const Symbol> &x,
  ...
  static RCP<const Basic> diff(const Sech &self, const RCP<const Symbol> &x) {
    return mul(mul(mul(minus_one, sech(self.get_arg())), tanh(self.get_arg())),
               self.get_arg()->diff(x));
  }
  ...
  static RCP<const Basic> diff(const Csch &self, const RCP<const Symbol> &x) {
    return mul(mul(mul(minus_one, csch(self.get_arg())), coth(self.get_arg())),
               self.get_arg()->diff(x));
  }
  ...
  static RCP<const Basic> diff(const Derivative &self,
    ...
    if (is_a<Derivative>(*ret) &&
        eq(*down_cast<const Derivative &>(*ret).get_arg(), *self.get_arg())) {
      t.insert(x);
      return Derivative::create(self.get_arg(), t);
    }
    for (auto &p : t) {
  ...
  static RCP<const Basic> diff(const LambertW &self,
    ...
    return mul(div(lambertw_val, mul(self.get_arg(), add(lambertw_val, one))),
               self.get_arg()->diff(x));
  }
  ...
  static RCP<const Basic> diff(const Add &self, const RCP<const Symbol> &x) {
    ...
    for (auto &p : self.get_dict()) {
      ...
      if (is_a<Integer>(*term) && down_cast<const Integer &>(*term).is_zero()) {
        continue;
  ...
  static RCP<const Basic> diff(const Csc &self, const RCP<const Symbol> &x) {
    return mul(mul(mul(cot(self.get_arg()), csc(self.get_arg())), minus_one),
               self.get_arg()->diff(x));
  }
