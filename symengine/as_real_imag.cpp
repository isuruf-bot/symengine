...
public:
  ...
  void bvisit(const Sin &x) {
    apply(*x.get_arg());
    std::tie(*real_, *imag_) = std::make_tuple(mul(sin(*real_), cosh(*imag_)),
                                               mul(sinh(*imag_), cos(*real_)));
  }

  void bvisit(const Cos &x) {
    apply(*x.get_arg());
    std::tie(*real_, *imag_) = std::make_tuple(
        mul(cos(*real_), cosh(*imag_)), neg(mul(sinh(*imag_), sin(*real_))));
  }
  ...
  void bvisit(const Csc &x) { apply(*div(one, sin(x.get_arg()))); }

  void bvisit(const Sec &x) { apply(*div(one, cos(x.get_arg()))); }
  ...
  void bvisit(const Sinh &x) {
    apply(*x.get_arg());
    std::tie(*real_, *imag_) = std::make_tuple(mul(sinh(*real_), cos(*imag_)),
                                               mul(sin(*imag_), cosh(*real_)));
  }

  void bvisit(const Cosh &x) {
    apply(*x.get_arg());
    std::tie(*real_, *imag_) = std::make_tuple(mul(cosh(*real_), cos(*imag_)),
                                               mul(sin(*imag_), sinh(*real_)));
  }
  ...
  void bvisit(const Csch &x) { apply(*div(one, sinh(x.get_arg()))); }

  void bvisit(const Sech &x) { apply(*div(one, cosh(x.get_arg()))); }
