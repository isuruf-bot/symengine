...
BooleanAtom::BooleanAtom(bool b) : b_{b} { SYMENGINE_ASSIGN_TYPEID() }
...
bool BooleanAtom::get_val() const { return b_; }

vec_basic BooleanAtom::get_args() const { return {}; }

bool BooleanAtom::__eq__(const Basic &o) const {
  return is_a<BooleanAtom>(o) and
         get_val() == down_cast<const BooleanAtom &>(o).get_val();
}
...
RCP<const Basic> Contains::get_expr() const { return expr_; }

RCP<const Set> Contains::get_set() const { return set_; }
...
bool Contains::__eq__(const Basic &o) const {
  return is_a<Contains>(o) and
         unified_eq(get_expr(), down_cast<const Contains &>(o).get_expr()) and
         unified_eq(get_set(), down_cast<const Contains &>(o).get_set());
}
...
const PiecewiseVec &Piecewise::get_vec() const { return vec_; }
...
bool Piecewise::__eq__(const Basic &o) const {
  return is_a<Piecewise>(o) and
         unified_eq(get_vec(), down_cast<const Piecewise &>(o).get_vec());
}
...
bool And::__eq__(const Basic &o) const {
  return is_a<And>(o) and
         unified_eq(container_, down_cast<const And &>(o).get_container());
}

int And::compare(const Basic &o) const {
  SYMENGINE_ASSERT(is_a<And>(o))
  return unified_compare(container_, down_cast<const And &>(o).get_container());
}
...
bool Or::__eq__(const Basic &o) const {
  return is_a<Or>(o) and
         unified_eq(container_, down_cast<const Or &>(o).get_container());
}

int Or::compare(const Basic &o) const {
  SYMENGINE_ASSERT(is_a<Or>(o))
  return unified_compare(container_, down_cast<const Or &>(o).get_container());
}
...
RCP<const Boolean> Not::get_arg() const { return arg_; }

RCP<const Boolean> Not::logical_not() const { return this->get_arg(); }
...
bool Xor::__eq__(const Basic &o) const {
  return is_a<Xor>(o) and
         unified_eq(container_, down_cast<const Xor &>(o).get_container());
}

int Xor::compare(const Basic &o) const {
  SYMENGINE_ASSERT(is_a<Xor>(o))
  return unified_compare(container_, down_cast<const Xor &>(o).get_container());
}
...
const vec_boolean &Xor::get_container() const { return container_; }
...
RCP<const Boolean> and_or(const set_boolean &s, const bool &op_x_notx) {
  ...
  if (not op_x_notx) {
    for (auto it = args.begin(); it != args.end(); it++) {
      if (is_a<Contains>(**it) and
          is_a<Symbol>(*down_cast<const Contains &>(**it).get_expr()) and
          is_a<FiniteSet>(*down_cast<const Contains &>(**it).get_set())) {
        ...
        if (not symexists) {
          ...
          restCond = logical_and({finiteset(present)->contains(sym), restCond});
          return restCond;
        } else {
...
RCP<const Boolean> logical_xor(const vec_boolean &s) {
  ...
  if (nots % 2 == 0) {
    ...
    if (args.size() == 0) {
      ...
      return make_rcp<const Not>(make_rcp<const Xor>(get_vec_from_set(args)));
    }
  }
}
...
RCP<const Boolean> Eq(const RCP<const Basic> &lhs) { return Eq(lhs, zero); }
...
RCP<const Boolean> Eq(const RCP<const Basic> &lhs,
  ...
  if (b) {
    ...
    if ((is_a_Number(*lhs) and is_a_Number(*rhs)) or
        (is_a<BooleanAtom>(*lhs) and is_a<BooleanAtom>(*rhs)))
      return boolean(false);
...
RCP<const Boolean> Ne(const RCP<const Basic> &lhs,
                      const RCP<const Basic> &rhs) {
...
RCP<const Boolean> Le(const RCP<const Basic> &lhs,
                      const RCP<const Basic> &rhs) {
  ...
  if (is_a_Number(*lhs) and is_a_Number(*rhs)) {
    RCP<const Number> s =
        down_cast<const Number &>(*lhs).sub(down_cast<const Number &>(*rhs));
    if (s->is_negative())
      return boolean(true);
    return boolean(false);
  }
...
RCP<const Boolean> Ge(const RCP<const Basic> &lhs,
                      const RCP<const Basic> &rhs) {
  return Le(rhs, lhs);
}
...
RCP<const Boolean> Lt(const RCP<const Basic> &lhs,
                      const RCP<const Basic> &rhs) {
  if (is_a_Complex(*lhs) or is_a_Complex(*rhs))
    throw SymEngineException("Invalid comparison of complex numbers.");
  if (is_a<NaN>(*lhs) or is_a<NaN>(*rhs))
...
RCP<const Boolean> Gt(const RCP<const Basic> &lhs,
                      const RCP<const Basic> &rhs) {
  return Lt(rhs, lhs);
}
