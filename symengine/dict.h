...
template <typename T> struct vec_hash { hash_t operator()(const T &v) const; };
...
template <typename T,
          typename = enable_if_t<std::is_arithmetic<T>::value or
                                 std::is_same<T, integer_class>::value>>
...
template <class T> bool unordered_eq(const T &a, const T &b) {
...
template <class T> bool ordered_eq(const T &A, const T &B) {
  // Can't be equal if # of entries differ:
  if (A.size() != B.size())
    return false;
...
template <typename T,
          typename = enable_if_t<std::is_arithmetic<T>::value or
                                 std::is_same<T, integer_class>::value or
                                 std::is_same<T, rational_class>::value>>
