...
public:
  ...
  void bvisit(const Basic &x) { result_ = x.rcp_from_this(); }
  ...
  void bvisit(const Add &x) {
    ...
    for (const auto &p : x.get_dict()) {
      auto it = subs_dict_.find(Add::from_dict(zero, {{p.first, p.second}}));
      if (it != subs_dict_.end()) {
        Add::coef_dict_add_term(outArg(coef), d, one, it->second);
      } else {
        ...
        if (it != subs_dict_.end()) {
          ...
          Add::coef_dict_add_term(outArg(coef), d, p.second, apply(p.first));
        }
      }
    }
  ...
  template <class T> void bvisit(const TwoArgBasic<T> &x) {
  ...
  void bvisit(const ImageSet &x) {
    ...
    if (s == x.get_symbol() and expr == x.get_expr() and
        bs == x.get_baseset()) {
      result_ = x.rcp_from_this();
    } else {
  ...
  RCP<const Basic> apply(const Basic &x) { return apply(x.rcp_from_this()); }
...
public:
  ...
  void bvisit(const Pow &x) {
    RCP<const Basic> base_new = apply(x.get_base());
    RCP<const Basic> exp_new = apply(x.get_exp());
    if (subs_dict_.size() == 1 and is_a<Pow>(*((*subs_dict_.begin()).first)) and
        not is_a<Add>(
            *down_cast<const Pow &>(*(*subs_dict_.begin()).first).get_exp())) {
      auto &subs_first = down_cast<const Pow &>(*(*subs_dict_.begin()).first);
  ...
  void bvisit(const Derivative &x) {
    ...
    for (const auto &p : subs_dict_) {
      ...
      if (is_a<Symbol>(*p.first) and is_a<Symbol>(*p.second) and
          eq(*x.get_arg()->diff(rcp_static_cast<const Symbol>(p.second)),
             *zero)) {
...
public:
  ...
  void bvisit(const Derivative &x) { result_ = x.rcp_from_this(); }
