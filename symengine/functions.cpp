...
bool Conjugate::is_canonical(const RCP<const Basic> &arg) const {
  ...
  if (is_a<Sign>(*arg) or is_a<Conjugate>(*arg) or is_a<Erf>(*arg) or
      is_a<Erfc>(*arg) or is_a<Gamma>(*arg) or is_a<LogGamma>(*arg) or
      is_a<Abs>(*arg)) {
    return false;
  }
  if (is_a<Sin>(*arg) or is_a<Cos>(*arg) or is_a<Tan>(*arg) or
      is_a<Cot>(*arg) or is_a<Sec>(*arg) or is_a<Csc>(*arg)) {
    return false;
  }
  if (is_a<Sinh>(*arg) or is_a<Cosh>(*arg) or is_a<Tanh>(*arg) or
      is_a<Coth>(*arg) or is_a<Sech>(*arg) or is_a<Csch>(*arg)) {
    return false;
  }
  // TwoArgFunction classes
  if (is_a<KroneckerDelta>(*arg) or is_a<ATan2>(*arg) or
      is_a<LowerGamma>(*arg) or is_a<UpperGamma>(*arg) or is_a<Beta>(*arg)) {
    return false;
  }
...
RCP<const Basic> conjugate(const RCP<const Basic> &arg) {
  ...
  if (is_a<Constant>(*arg) or is_a<Abs>(*arg) or is_a<KroneckerDelta>(*arg) or
      is_a<LeviCivita>(*arg)) {
    return arg;
  }
  if (is_a<Mul>(*arg)) {
  ...
  if (is_a<Sign>(*arg) or is_a<Erf>(*arg) or is_a<Erfc>(*arg) or
      is_a<Gamma>(*arg) or is_a<LogGamma>(*arg) or is_a<Sin>(*arg) or
      is_a<Cos>(*arg) or is_a<Tan>(*arg) or is_a<Cot>(*arg) or
      is_a<Sec>(*arg) or is_a<Csc>(*arg) or is_a<Sinh>(*arg) or
      is_a<Cosh>(*arg) or is_a<Tanh>(*arg) or is_a<Coth>(*arg) or
      is_a<Sech>(*arg) or is_a<Csch>(*arg)) {
    const OneArgFunction &func = down_cast<const OneArgFunction &>(*arg);
    return func.create(conjugate(func.get_arg()));
  }
  if (is_a<ATan2>(*arg) or is_a<LowerGamma>(*arg) or is_a<UpperGamma>(*arg) or
      is_a<Beta>(*arg)) {
    const TwoArgFunction &func = down_cast<const TwoArgFunction &>(*arg);
    return func.create(conjugate(func.get_arg1()), conjugate(func.get_arg2()));
  }
...
bool get_pi_shift(const RCP<const Basic> &arg, const Ptr<RCP<const Number>> &n,
  ...
  if (is_a<Add>(*arg)) {
    ...
    if (size > 1) {
      ...
      *x = coef;
      for (const auto &p : s.get_dict()) {
        if (eq(*p.first, *pi) and
            (is_a<Integer>(*p.second) or is_a<Rational>(*p.second))) {
          check_pi = true;
          *n = p.second;
        } else {
    ...
    if (s.get_dict().size() == 1 and eq(*p->first, *pi) and
        eq(*p->second, *one) and
        (is_a<Integer>(*s.get_coef()) or is_a<Rational>(*s.get_coef()))) {
      *n = s.get_coef();
      *x = zero;
      return true;
    } else {
    ...
    for (const auto &p : s.get_dict()) {
      ...
      if (eq(*p.first, *pi)) {
        ...
        if (is_a<Rational>(*temp)) {
          auto m = down_cast<const Rational &>(*temp).as_rational_class();
          return (m < 0) or (m > 1);
        }
        return false;
      }
...
bool could_extract_minus(const Basic &arg) {
  if (is_a_Number(arg)) {
    if (down_cast<const Number &>(arg).is_negative()) {
      ...
      return (real_part->is_negative()) or
             (eq(*real_part, *zero) and c.imaginary_part()->is_negative());
    } else {
      return false;
    }
  } else if (is_a<Mul>(arg)) {
...
bool Sign::is_canonical(const RCP<const Basic> &arg) const {
  ...
  if (is_a<Mul>(*arg)) {
    if (neq(*down_cast<const Mul &>(*arg).get_coef(), *one) and
...
RCP<const Basic> sign(const RCP<const Basic> &arg) {
  if (is_a_Number(*arg)) {
    ...
    if (is_a_Complex(*arg) and
        down_cast<const ComplexBase &>(*arg).is_re_zero()) {
      RCP<const Number> r =
  ...
  if (is_a<Constant>(*arg)) {
    if (eq(*arg, *pi) or eq(*arg, *E) or eq(*arg, *EulerGamma) or
        eq(*arg, *Catalan) or eq(*arg, *GoldenRatio))
      return one;
  }
  if (is_a<Sign>(*arg)) {
    return arg;
  ...
  if (is_a<Mul>(*arg)) {
    ...
    return mul(s, make_rcp<const Sign>(Mul::from_dict(one, std::move(dict))));
  }
...
RCP<const Basic> floor(const RCP<const Basic> &arg) {
  ...
  if (is_a<BooleanAtom>(*arg) or is_a_Relational(*arg)) {
    throw SymEngineException("Boolean objects not allowed in this context.");
  }
  if (is_a<Add>(*arg)) {
    RCP<const Number> s = down_cast<const Add &>(*arg).get_coef();
    umap_basic_num d = down_cast<const Add &>(*arg).get_dict();
    if (is_a<Integer>(*s)) {
      return add(s, make_rcp<const Floor>(Add::from_dict(zero, std::move(d))));
    }
  }
...
RCP<const Basic> ceiling(const RCP<const Basic> &arg) {
  ...
  if (is_a<Add>(*arg)) {
    ...
    if (is_a<Integer>(*s)) {
      return add(s,
                 make_rcp<const Ceiling>(Add::from_dict(zero, std::move(d))));
    }
  }
...
RCP<const Basic> asin(const RCP<const Basic> &arg) {
  ...
  else if (is_a_Number(*arg) and
           not down_cast<const Number &>(*arg).is_exact()) {
...
RCP<const Basic> Log::create(const RCP<const Basic> &a) const { return log(a); }
...
RCP<const Basic> log(const RCP<const Basic> &arg) {
  ...
  if (is_a<Rational>(*arg)) {
    ...
    get_num_den(down_cast<const Rational &>(*arg), outArg(num), outArg(den));
  ...
  if (is_a<Complex>(*arg)) {
    ...
    if (_arg->is_re_zero()) {
      ...
      if (arg_img->is_negative()) {
        return sub(log(mul(minus_one, arg_img)), mul(I, div(pi, integer(2))));
...
bool FunctionSymbol::__eq__(const Basic &o) const {
  if (is_a<FunctionSymbol>(o) and
      name_ == down_cast<const FunctionSymbol &>(o).name_ and
      unified_eq(get_vec(), down_cast<const FunctionSymbol &>(o).get_vec()))
    return true;
  return false;
}
...
bool Derivative::is_canonical(const RCP<const Basic> &arg,
  ...
  if (is_a<FunctionSymbol>(*arg) or is_a<LeviCivita>(*arg)) {
    ...
    return true;
  } else if (is_a<PolyGamma>(*arg) or is_a<Zeta>(*arg) or
             is_a<UpperGamma>(*arg) or is_a<LowerGamma>(*arg) or
             is_a<Dirichlet_eta>(*arg)) {
    bool found = false;
    auto v = arg->get_args();
    for (auto &p : x) {
...
bool Derivative::__eq__(const Basic &o) const {
  if (is_a<Derivative>(o) and
      eq(*arg_, *(down_cast<const Derivative &>(o).arg_)) and
      unified_eq(x_, down_cast<const Derivative &>(o).x_))
    return true;
  return false;
}
...
bool Subs::__eq__(const Basic &o) const {
  if (is_a<Subs>(o) and eq(*arg_, *(down_cast<const Subs &>(o).arg_)) and
      unified_eq(dict_, down_cast<const Subs &>(o).dict_))
    return true;
  return false;
}
...
bool Gamma::is_canonical(const RCP<const Basic> &arg) const {
  ...
  if (is_a<Rational>(*arg) and
      (get_den(down_cast<const Rational &>(*arg).as_rational_class())) == 2) {
    return false;
  }
  if (is_a_Number(*arg) and not down_cast<const Number &>(*arg).is_exact()) {
    return false;
...
bool LowerGamma::is_canonical(const RCP<const Basic> &s,
  ...
  if (is_a<Integer>(*s) and
      down_cast<const Integer &>(*s).as_integer_class() > 1)
    return false;
...
RCP<const Basic> lowergamma(const RCP<const Basic> &s,
  ...
  if (is_a<Integer>(*s)) {
    ...
    if (eq(*s, *div(one, integer(2)))) {
      return mul(sqrt(pi), erf(sqrt(x))); // base case for s of the form n/2
...
RCP<const Basic> uppergamma(const RCP<const Basic> &s,
  ...
  if (is_a<Integer>(*s)) {
    ...
    s_num = subnum(s_num, one);
    if (eq(*s, *div(one, integer(2)))) {
      return mul(sqrt(pi), erfc(sqrt(x))); // base case for s of the form n/2
    } else if (s_num->is_positive()) {
...
bool Beta::is_canonical(const RCP<const Basic> &x, const RCP<const Basic> &y) {
  ...
  if (is_a<Integer>(*x) or
      (is_a<Rational>(*x) and
       (get_den(down_cast<const Rational &>(*x).as_rational_class())) == 2)) {
    if (is_a<Integer>(*y) or
        (is_a<Rational>(*y) and
         (get_den(down_cast<const Rational &>(*y).as_rational_class())) == 2)) {
      return false;
    }
  }
  return true;
    ...
    if (x_int->is_positive()) {
      if (is_a<Integer>(*y)) {
        ...
        if (y_int->is_positive()) {
          return div(mul(gamma_positive_int(x), gamma_positive_int(y)),
...
RCP<const Basic> polygamma(const RCP<const Basic> &n_,
  ...
  // Only special values are being evaluated
  if (is_a_Number(*x_) and not(down_cast<const Number &>(*x_)).is_positive()) {
    return ComplexInf;
  }
...
RCP<const Basic> Max::create(const vec_basic &a) const { return max(a); }
...
RCP<const Basic> Min::create(const vec_basic &a) const { return min(a); }
