...
public:
  ...
  void add_to_gen_set(const RCP<const Basic> &base,
    ...
    if (is_a<const Rational>(*exp)) {
      RCP<const Integer> den = down_cast<const Rational &>(*exp).get_den();
      if (is_a<const Rational>(*it->second))
        gen_set[base] = divnum(
            one,
            lcm(*den, *down_cast<const Rational &>(*it->second).get_den()));
      else
...
public:
  ...
  void bvisit(const Basic &x) { gen_set[x.rcp_from_this()] = one; }
};
