...
public:
  ...
  int compare(const Basic &o) const {
    ...
    for (unsigned int i = 0; i < this->get_poly().length(); ++i) {
      if (this->get_poly().get_coeff(i) != s.get_poly().get_coeff(i))
        return (this->get_poly().get_coeff(i) < s.get_poly().get_coeff(i)) ? -1
                                                                           : 1;
    }
    return 0;
  }
  ...
  int size() const { return numeric_cast<int>(this->get_poly().length()); }
};
...
factors(const UFlintPoly<Container, BaseType, Poly> &a) {
  ...
  for (long i = 0; i < fac->num; i++) {
    ...
    S.push_back(std::make_pair(make_rcp<const Poly>(a.get_var(), std::move(z)),
                               fac->exp[i]));
  }
  return S;
}
