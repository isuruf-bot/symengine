...
template <typename Vec, typename Value, typename Wrapper> class UDictWrapper {
public:
  ...
  UDictWrapper(unsigned int s) SYMENGINE_NOEXCEPT { vec_size = s; }
  ...
  Wrapper &operator*=(const Wrapper &other) {
    ...
    if (other.dict_.size() == 1 and
        other.dict_.find(zero_v) != other.dict_.end()) {
  ...
  const Dict &get_dict() const { return dict_; }
...
public:
  ...
  inline const set_basic &get_vars() const { return vars_; }
...
public:
  ...
  IMPLEMENT_TYPEID(MINTPOLY) hash_t __hash__() const;
...
template <typename Poly> RCP<const Poly> neg_mpoly(const Poly &a) {
  auto x = a.get_poly();
