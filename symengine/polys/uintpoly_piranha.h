...
struct pow_impl<V, U, enable_if_t<std::is_integral<U>::value and
                                  (std::is_same<V, integer_class>::value or
                                   std::is_same<V, rational_class>::value)>> {
  template <typename T2> V operator()(const V &r, const T2 &x) const {
    V res;
    mp_pow_ui(res, r, x);
    return res;
  }
...
template <> struct divexact_impl<SymEngine::integer_class> {
...
template <> struct divexact_impl<SymEngine::rational_class> {
...
template <> struct has_exact_ring_operations<SymEngine::integer_class> {
  static const bool value = true;
};
template <> struct has_exact_ring_operations<SymEngine::rational_class> {
  static const bool value = true;
};
}
#endif
...
template <typename Cf, typename Container> class PiranhaForIter {
public:
...
public:
  ...
  bool operator==(const PiranhaForIter &rhs) { return (ptr_ == rhs.ptr_); }
...
public:
  ...
  iterator end() const { return iterator(this->get_poly()._container().end()); }
