...
public:
  ...
  inline Cf get_coeff(Key x) const { return this->get_poly().get_coeff(x); }

  typedef typename std::map<Key, Cf>::const_iterator iterator;
  typedef typename std::map<Key, Cf>::const_reverse_iterator r_iterator;
  iterator begin() const { return this->get_poly().dict_.begin(); }
  iterator end() const { return this->get_poly().dict_.end(); }
  r_iterator obegin() const { return this->get_poly().dict_.rbegin(); }
  r_iterator oend() const { return this->get_poly().dict_.rend(); }
