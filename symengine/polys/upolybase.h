...
inline integer_class to_mp_class(const integer_class &i) { return i; }

inline rational_class to_mp_class(const rational_class &i) { return i; }

// dict wrapper
template <typename Key, typename Value, typename Wrapper> class ODictWrapper {
public:
...
public:
  ...
  ODictWrapper(std::string s) { dict_[1] = Value(1); }
  ...
  template <typename FromPoly> static Wrapper from_poly(const FromPoly &p) {
    Wrapper t;
  ...
  bool operator==(const Wrapper &other) const { return dict_ == other.dict_; }

  bool operator!=(const Wrapper &other) const { return not(*this == other); }

  const std::map<Key, Value> &get_dict() const { return dict_; }
...
template <typename Container, typename Poly> class UPolyBase : public Basic {
private:
...
public:
  ...
  inline bool __eq__(const Basic &o) const {
    if (is_a<Poly>(o))
      return eq(*var_, *(down_cast<const Poly &>(o).var_)) and
             poly_ == down_cast<const Poly &>(o).poly_;
    return false;
  }

  inline const RCP<const Basic> &get_var() const { return var_; }

  inline const Container &get_poly() const { return poly_; }

  inline vec_basic get_args() const { return {}; }
...
public:
  ...
  inline int get_degree() const { return this->get_poly().degree(); }
    ...
    for (; it != end; ++it) {
      if (it->first == 0)
        ...
        if (it->second == Expression(1))
          ...
          args.push_back(mul(it->second.get_basic(), this->get_var()));
      } else if (it->second == 1)
...
public:
  ...
  Cf get_lc() const { return get_coeff(get_degree()); }
    ...
    for (; it != end; ++it) {
      ...
      if (it->first == 0) {
        ...
        if (m == 1) {
          ...
          args.push_back(Mul::from_dict(integer(m), {{this->get_var(), one}}));
        }
      } else {
        if (m == 1) {
          args.push_back(pow(this->get_var(), integer(it->first)));
    ...
    for (; it != end; ++it) {
      ...
      if (it->first == 0) {
        ...
        if (m == 1) {
          ...
          args.push_back(
              Mul::from_dict(Rational::from_mpq(m), {{this->get_var(), one}}));
        }
      } else {
        if (m == 1) {
          args.push_back(pow(this->get_var(), integer(it->first)));
          ...
          args.push_back(Mul::from_dict(
              Rational::from_mpq(m), {{this->get_var(), integer(it->first)}}));
        }
      }
    }
    return SymEngine::add(args);
...
template <typename T, typename Int> class ContainerBaseIter {
protected:
  RCP<const T> ptr_;
  long i_;
...
public:
  ...
  bool operator!=(const ContainerBaseIter &rhs) { return not(*this == rhs); }
...
public:
  ContainerForIter(RCP<const T> ptr, long x)
    ...
    if (this->ptr_->get_coeff_ref(numeric_cast<unsigned>(this->i_)) == 0 and
        this->i_ < this->ptr_->size()) {
      ++(*this);
    }
  }
...
template <typename P> struct is_a_UPoly {
