...
public:
  ...
  BasicToUPolyBase(const RCP<const Basic> &gen_) { gen = gen_; }
  ...
  void bvisit(const Pow &x) {
    if (is_a<const Integer>(*x.get_exp())) {
      int i =
          numeric_cast<int>(down_cast<const Integer &>(*x.get_exp()).as_int());
      if (i > 0) {
  ...
  template <
      ...
      typename = enable_if_t<
          ((std::is_base_of<UIntPolyBase<typename P::container_type, P>,
                            P>::value and
            std::is_base_of<UIntPolyBase<typename Poly::container_type, Poly>,
                            Poly>::value) or
           (std::is_base_of<URatPolyBase<typename P::container_type, P>,
                            P>::value and
            (std::is_base_of<UIntPolyBase<typename Poly::container_type, Poly>,
                             Poly>::value or
             std::is_base_of<URatPolyBase<typename Poly::container_type, Poly>,
                             Poly>::value)) or
           (std::is_same<P, UExprPoly>::value and
            std::is_base_of<UPolyBase<typename Poly::container_type, Poly>,
                            Poly>::value)) and
          not std::is_same<Poly, GaloisField>::value>>
  ...
  void bvisit(const Basic &x) {
    ...
    if (eq(*genbase, x)) {
      ...
      if (is_a<const Integer>(*powr)) {
        int i = numeric_cast<int>(down_cast<const Integer &>(*powr).as_int());
        if (i > 0) {
          dict = P::container_from_dict(gen, {{i, typename P::coef_type(1)}});
          return;
        }
      }
    }
  ...
  void dict_set(unsigned int pow, const Basic &x) {
    if (is_a<const Integer>(x))
      this->dict = Poly::container_from_dict(
          this->gen, {{pow, down_cast<const Integer &>(x).as_integer_class()}});
    else
      throw SymEngineException("Non-integer found");
  }
};
...
public:
  ...
  void bvisit(const Rational &x) { dict = UExprDict(x.rcp_from_this()); }
      ...
      for (auto pows : it->second) {
        ...
        if (is_a<const Integer>(*powr)) {
          ...
          if (i > 0) {
            ...
            dict = P::container_from_dict(gens,
                                          {{zero_v, typename P::coef_type(1)}});
            return;
          }
        }
      }
  ...
  void dict_set(vec_uint pow, const Basic &x) {
    if (is_a<const Integer>(x))
      dict = MIntPoly::container_from_dict(
          gens, {{pow, down_cast<const Integer &>(x).as_integer_class()}});
    else
      throw SymEngineException("Non-integer found");
  }
};
