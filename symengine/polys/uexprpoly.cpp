...
bool UExprPoly::is_zero() const { return get_poly().empty(); }

bool UExprPoly::is_one() const {
  return get_poly().size() == 1 and
         get_poly().get_dict().begin()->second == 1 and
         get_poly().get_dict().begin()->first == 0;
}
...
bool UExprPoly::is_symbol() const {
  return get_poly().size() == 1 and
         get_poly().get_dict().begin()->first == 1 and
         get_poly().get_dict().begin()->second == 1;
}

bool UExprPoly::is_mul() const {
  return get_poly().size() == 1 and
         get_poly().get_dict().begin()->first != 0 and
         get_poly().get_dict().begin()->second != 1 and
         get_poly().get_dict().begin()->second != 0;
}
