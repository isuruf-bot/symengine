...
TEST_CASE("And, Or : Basic", "[basic]") {
  ...
  REQUIRE(
      eq(*logical_or({c1, logical_and({c2, c3, c4}), logical_and({c2, c4}),
...
TEST_CASE("Xor : Basic", "[basic]") {
  ...
  REQUIRE(
      eq(*logical_xor({boolFalse, boolFalse, boolFalse, boolTrue}), *boolTrue));
  ...
  REQUIRE(eq(*logical_xor({boolTrue, c1, logical_not(c1), c2, logical_not(c2)}),
             *boolTrue));
