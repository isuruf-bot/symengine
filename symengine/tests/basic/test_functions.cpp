...
TEST_CASE("Derivative: functions", "[functions]") {
  ...
  std::cout << *f << " " << *r1 << std::endl;
  r2 = Subs::create(Derivative::create(function_symbol("f", __xi_1), {__xi_1}),
                    {{__xi_1, add(_xi_1, x)}});
  REQUIRE(eq(*r1, *r2));
  ...
  r2 = r1->subs({{x, y}});
  r3 = Subs::create(
      Derivative::create(function_symbol("f", {x, add(y, y)}), {x}), {{x, y}});
  REQUIRE(eq(*r2, *r3));
...
TEST_CASE("Subs: functions", "[functions]") {
  ...
  r2 = r1->diff(x);
  r3 = Subs::create(Derivative::create(function_symbol("f", {add(y, y), _xi_2}),
                                       {_xi_2, _xi_2}),
                    {{_xi_2, add(x, y)}});
  REQUIRE(eq(*r2, *r3));
  ...
  r3 = Subs::create(Derivative::create(function_symbol("f", {add(y, y), _xi_2}),
                    ...
                    {{_xi_2, add(x, y)}});
  r4 = Subs::create(
      Derivative::create(function_symbol("f", {_xi_1, _xi_2}), {_xi_1, _xi_2}),
...
TEST_CASE("Acos: functions", "[functions]") {
  ...
  REQUIRE(is_a<ComplexDouble>(*r2));
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r1).i) -
                   1.90908861124732) < 1e-12);
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r2).i) -
                   2.06343706889556) < 1e-12);
...
TEST_CASE("Asec: functions", "[functions]") {
  ...
  REQUIRE(is_a<ComplexDouble>(*r2));
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r1).i) -
                   1.44015500855881) < 1e-12);
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r2).i) -
                   1.31695789692482) < 1e-12);
...
TEST_CASE("Acsc: functions", "[functions]") {
  ...
  REQUIRE(is_a<ComplexDouble>(*r2));
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r1).i) -
                   0.438156111929239) < 1e-12);
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r2).i) -
                   2.21861690006402) < 1e-12);
...
TEST_CASE("Atanh: functions", "[functions]") {
  ...
  REQUIRE(is_a<ComplexDouble>(*r2));
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r1).i) -
                   1.09390752881482) < 1e-12);
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r2).i) -
                   1.66407281705924) < 1e-12);
...
TEST_CASE("Asech: functions", "[functions]") {
  ...
  REQUIRE(is_a<ComplexDouble>(*r1));
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r1).i) -
                   3.40646187463796) < 1e-12);

  r1 = asech(complex_double(std::complex<double>(1, 1)));
  REQUIRE(is_a<ComplexDouble>(*r1));
  REQUIRE(std::abs(std::abs(down_cast<const ComplexDouble &>(*r1).i) -
                   1.2380058304943624757) < 1e-12);
...
TEST_CASE("Zeta: functions", "[functions]") {
  ...
  r2 =
      add(mul(mul(mul(mul(pow(i2, x), pow(x, i2)), log(i2)),
                  zeta(add(pow(x, i2), i1), pow(i2, x))),
              im1),
          mul(mul(i2, x),
              Subs::create(Derivative::create(zeta(_xi_1, pow(i2, x)), {_xi_1}),
                           {{_xi_1, pow(x, i2)}})));
  REQUIRE(eq(*r1, *r2));
...
TEST_CASE("Levi Civita: functions", "[functions]") {
  ...
  r2 = add(mul(mul(i2, x),
               Subs::create(Derivative::create(levi_civita({_xi_1, mul(i2, x)}),
                                               {_xi_1}),
                            {{_xi_1, pow(x, i2)}})),
...
TEST_CASE("Beta: functions", "[functions]") {
  ...
  r2 = mul(
      ...
      add(mul(mul(i2, x), polygamma(zero, mul(x, x))),
          sub(polygamma(zero, x),
              mul(add(mul(i2, x), one), polygamma(zero, add(x, mul(x, x)))))));
  REQUIRE(eq(*r1, *r2));
...
TEST_CASE("Polygamma: functions", "[functions]") {
  ...
  r2 = mul(i2, Subs::create(Derivative::create(polygamma(_x, mul(i3, x)), {_x}),
                            {{_x, mul(i2, x)}}));
...
TEST_CASE("MPFR and MPC: functions", "[functions]") {
  ...
  std::vector<
      std::tuple<RCP<const Basic>, integer_class, integer_class>> testvec = {
      std::make_tuple(sin(real_mpfr(b1)), 841470984807896_z, 841470984807897_z),
      std::make_tuple(sin(sub(div(pi, i2), real_mpfr(b2))), -416146836547143_z,
                      -416146836547142_z),
      std::make_tuple(cos(real_mpfr(b1)), 540302305868139_z, 540302305868140_z),
      ...
      std::make_tuple(log(real_mpfr(b4)), 788457360364270_z, 788457360364271_z),
      std::make_tuple(gamma(div(real_mpfr(b3), i2)), 886226925452758_z,
                      886226925452759_z),
      std::make_tuple(exp(real_mpfr(b4)), 9025013499434122_z,
                      9025013499434123_z),
      std::make_tuple(erf(real_mpfr(b2)), 995322265018952_z, 995322265018953_z),
      std::make_tuple(erfc(real_mpfr(b2)), 4677734981047_z, 4677734981048_z),
  };
  ...
  for (unsigned i = 0; i < testvec.size(); i++) {
    ...
    mpfr_mul_z(a.get_mpfr_t(), down_cast<const RealMPFR &>(*r1).i.get_mpfr_t(),
               get_mpz_t(p), MPFR_RNDN);
    REQUIRE(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(std::get<1>(testvec[i]))) > 0);
    REQUIRE(mpfr_cmp_z(a.get_mpfr_t(), get_mpz_t(std::get<2>(testvec[i]))) < 0);
  }
  testvec.clear();
  ...
  testvec = {
      ...
      std::make_tuple(tan(complex_mpc(d2)), 1003245688405081_z,
                      1003245688405082_z),
      std::make_tuple(csc(complex_mpc(d2)), 99412891287795_z, 99412891287796_z),
      std::make_tuple(sec(complex_mpc(d2)), 99735554556364_z, 99735554556365_z),
...
TEST_CASE("test_conjugate", "[Conjugate]") {
  ...
  CHECK(eq(
      *r, *pow(Complex::from_two_nums(*integer(2), *integer(-3)), integer(2))));
  ...
  r = conjugate(mul(mul(integer(2), x), pow(y, Rational::from_two_ints(3, 2))));
  s = mul(integer(2),
          mul(conjugate(x), conjugate(pow(y, Rational::from_two_ints(3, 2)))));
  CHECK(eq(*r, *s));

  r = conjugate(mul(mul(complex_double(std::complex<double>(2.0, 3.0)), x), y));
...
TEST_CASE("test rewrite_as_exp", "[Functions]") {
  ...
  r2 = add(mul({rational(-1, 2), I, sub(exp(mul(I, x)), exp(mul(neg(I), x)))}),
  ...
  r2 = log(mul({rational(-1, 2), I, sub(exp(mul(I, x)), exp(mul(neg(I), x)))}));
  REQUIRE(eq(*r1, *r2));

  r1 = rewrite_as_exp(zeta(sin(x), cos(x)));
  r2 = zeta(mul({rational(-1, 2), I, sub(exp(mul(I, x)), exp(mul(neg(I), x)))}),
            mul(rational(1, 2), add(exp(mul(I, x)), exp(mul(neg(I), x)))));
  REQUIRE(eq(*r1, *r2));
