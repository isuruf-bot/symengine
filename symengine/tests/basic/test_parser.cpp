...
TEST_CASE("Parsing: functions", "[parser]") {
  ...
  res = parse(s);
  REQUIRE(eq(
      *res, *add(integer(2), add(zeta(integer(2), x), zeta(log(integer(3)))))));
  ...
  s = "(x < y) & (w >= z) | (y == z)";
  res = parse(s);
  REQUIRE(eq(*res, *logical_or({logical_and({Lt(x, y), Le(z, w)}), Eq(y, z)})));

  s = "(x < y) & ((w >= z) | (y == z))";
  res = parse(s);
  REQUIRE(eq(*res, *logical_and({logical_or({Eq(y, z), Le(z, w)}), Lt(x, y)})));
  ...
  s = "(x < y) | (w >= z) ^ (y == z)";
  res = parse(s);
  REQUIRE(eq(*res, *logical_or({Lt(x, y), logical_xor({Le(z, w), Eq(y, z)})})));
