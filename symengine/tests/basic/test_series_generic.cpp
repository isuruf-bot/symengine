...
TEST_CASE("Integration of UnivariateSeries", "[UnivariateSeries]") {
  ...
  REQUIRE_THROWS_AS(UnivariateSeries::integrate(a, UnivariateSeries::var("x")),
                    NotImplementedError);
  REQUIRE(UnivariateSeries::integrate(b, UnivariateSeries::var("x")) == c);
}

TEST_CASE("UnivariateSeries: compare, as_basic, as_dict",
          "[UnivariateSeries]") {
  RCP<const Symbol> x = symbol("x");
  UExprDict P({{0, 1}, {1, 2}});
...
TEST_CASE("Expression series expansion: sin, cos", "[Expansion of sin, cos]") {
  ...
  REQUIRE(
      series_coeff(z7, x, 10, 9)
          ->__eq__(*mul((pow(symbol("a"), integer(9))), rational(1, 362880))));
}
...
TEST_CASE("Expression series expansion: sinh, cosh, tanh, asinh, atanh",
  ...
  auto ans6 = add(add(pow(tanh(integer(5)), integer(5)), // tanh(5)**5
                      mul(rational(-5, 3), pow(tanh(integer(5)),
                                               integer(3)))), //- 5*tanh(5)**3/3
