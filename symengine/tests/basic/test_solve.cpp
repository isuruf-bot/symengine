...
TEST_CASE("Higher order(degree >=5) polynomials", "[Solve]") {
  ...
  REQUIRE(temp.find(conditionset(
              x, Eq(add({mul(cbx, cbx), cbx, one}), zero))) != temp.end());
...
TEST_CASE("solve_poly", "[Solve]") {
  ...
  auto P3 = URatPolyFlint::from_dict(
      x,
      {{0, 1_q}, {1, rational_class(3_z, 2_z)}, {2, rational_class(1_z, 2_z)}});
  soln = solve_poly(P3, x);
  REQUIRE(eq(*soln, *finiteset({neg(one), neg(integer(2))})));
#endif
  ...
  auto P4 = URatPolyPiranha::from_dict(
      x,
      {{0, 1_q}, {1, rational_class(3_z, 2_z)}, {2, rational_class(1_z, 2_z)}});
  soln = solve_poly(P4, x);
  REQUIRE(eq(*soln, *finiteset({neg(one), neg(integer(2))})));
...
TEST_CASE("linsolve", "[Solve]") {
  ...
  solns = linsolve({add({x, mul(integer(2), y), mul(integer(3), z),
  ...
  solns = linsolve(
      {Eq(add({x, mul(integer(2), y), mul(integer(3), z), mul(integer(4), t)}),
          integer(10)),
  ...
  solns = linsolve(
      {add({mul(a, x), mul(b, y), e}), add({mul(c, x), mul(d, y), f})}, {x, y});
...
TEST_CASE("linear_eqns_to_matrix", "[Solve]") {
  ...
  std::pair<DenseMatrix, DenseMatrix> solns = linear_eqns_to_matrix(
      ...
      {x, y, z});
  REQUIRE(solns.first ==
          DenseMatrix(3, 3, {integer(1), integer(2), integer(3), integer(2),
  ...
  REQUIRE(solns.first ==
          DenseMatrix(3, 3, {integer(2), integer(1), integer(3), integer(2),
                             integer(2), integer(3), integer(3), integer(3),
                             integer(3)}));
  REQUIRE(solns.second == DenseMatrix(3, 1, {zero, zero, zero}));
...
TEST_CASE("trigonometric equations", "[Solve]") {
  ...
  req = set_union(
      {imageset(n, sub(mul({i2, n, pi}), div(pi, integer(4))),
                interval(NegInf, Inf, true, true)),
       imageset(n, add(mul({i2, n, pi}), div(mul(integer(3), pi), integer(4))),
                interval(NegInf, Inf, true, true))});
