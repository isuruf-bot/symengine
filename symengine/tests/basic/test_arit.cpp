...
TEST_CASE("Mul: arit", "[arit]") {
  ...
  r1 = mul(
      mul(mul(mul(div(i3, i2), pow(cos(pow(x, i2)), im2)), x), sin(pow(x, i2))),
      cos(div(mul(i3, x), i4)));
  r2 = mul(
      mul(mul(mul(div(i3, i2), pow(cos(pow(x, i2)), im2)), x), sin(pow(x, i2))),
      cos(div(mul(i3, x), i4)));
  ...
  REQUIRE(std::abs(down_cast<const ComplexDouble &>(*s3).i.real() -
                   down_cast<const ComplexDouble &>(*s2).i.real()) < 1e-12);
  REQUIRE(std::abs(down_cast<const ComplexDouble &>(*s3).i.imag() -
                   down_cast<const ComplexDouble &>(*s2).i.imag()) < 1e-12);
  ...
  s2 = complex_double(std::complex<double>(0.27395725383012, 0.58370075875861));
  ...
  s2 = complex_double(std::complex<double>(1.53847780272794, 1.27792255262727));
...
TEST_CASE("Pow: arit", "[arit]") {
  ...
  REQUIRE(is_a<ComplexDouble>(*r2));
  REQUIRE(std::abs(down_cast<const ComplexDouble &>(*r2).i.real() -
                   0.997598696589298) < 1e-12);
  REQUIRE(std::abs(down_cast<const ComplexDouble &>(*r2).i.imag() -
                   0.069259227279362) < 1e-12);
...
TEST_CASE("Expand1: arit", "[arit]") {
  ...
  std::cout << "number of terms: "
            << rcp_dynamic_cast<const Add>(r2)->get_dict().size() << std::endl;
}
...
TEST_CASE("Expand2: arit", "[arit]") {
  ...
  REQUIRE(
      eq(*r2, *pow(add(add(pow(x, i2), mul(mul(i2, x), y)), pow(y, i2)), im1)));
  REQUIRE(neq(*r2, *r1));
