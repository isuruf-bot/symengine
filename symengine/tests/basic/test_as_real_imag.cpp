...
TEST_CASE("RealImag: Mul", "[as_real_imag]") {
  ...
  as_real_imag(mul(add(i2, I), pow(I, rational(2, 3))), outArg(re), outArg(im));
...
TEST_CASE("RealImag: Add", "[as_real_imag]") {
  ...
  as_real_imag(add(add(i2, I), pow(I, rational(2, 3))), outArg(re), outArg(im));
...
TEST_CASE("RealImag: Trigonometric functions", "[as_real_imag]") {
  ...
  REQUIRE(eq(*re, *div(neg(sin(integer(4))), sub(cos(integer(4)), cosh(i2)))));
