...
TEST_CASE("CSE: simple", "[cse]") {
...

  {
    ...
    REQUIRE(
        unified_eq(reduced, {add({mul(x0, y), mul(x0, z), mul({i2, y, z})})}));
  }
  {
  ...
  }
  {
    ...
    cse(rsubsts, rreduced, {e3, e2, e1});
    REQUIRE(
        unified_eq(substs, {{x0, neg(z)}, {x1, add(x, x0)}, {x2, add(x0, y)}}));
  ...
  }
  {
    ...
    REQUIRE(
        unified_eq(reduced, {add(x, add(w, add(y, add(z, x0)))), mul(x0, x)}));
  }
  {
    auto e1 = add(x, y);
  ...
  }
  {
    ...
    REQUIRE(unified_eq(substs, {{x0, pow(x, i2)}}));
    REQUIRE(
        unified_eq(reduced, {add(div(y, add(x0, i2)), div(z, mul(x0, y)))}));
  }
  {
    ...
    cse(substs, reduced, {e});
    REQUIRE(unified_eq(substs, {{x0, pow(x, i2)}}));
    REQUIRE(unified_eq(reduced, {add(exp(x0), mul(x0, cos(pow(x0, im1))))}));
  }
  {
  ...
  }
  {
    ...
    REQUIRE(
        unified_eq(reduced, {add(x1, cos(x1)), add(x4, cos(x4)), add(x0, x2)}));
  }
  {
    auto e1 = add(x, i3);
    auto e2 = add(x, i4);
