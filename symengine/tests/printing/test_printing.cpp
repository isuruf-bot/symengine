...
TEST_CASE("test_floats(): printing", "[printing]") {
  ...
  p = add(p, x);
  pr = (p->__str__() == "1.0 + 1e-17*I + x") or
       (p->__str__() == "1.0 + 1e-017*I + x");
  REQUIRE(pr == true);

  p = complex_double(
      std::complex<double>(0.00000000000000001, 0.00000000000000001));
  p = add(p, x);
  pr = (p->__str__() == "1e-17 + 1e-17*I + x") or
       (p->__str__() == "1e-017 + 1e-017*I + x");
  REQUIRE(pr == true);
...
TEST_CASE("test_sets(): printing", "[printing]") {
  ...
  r1 = conditionset({x},
                    logical_and({i1->contains(x), Ge(mul(x, x), integer(9))}));
...
TEST_CASE("test_conjugate(): printing", "[printing]") {
  ...
  RCP<const Symbol> y = symbol("y");
  RCP<const Basic> r =
      conjugate(mul(mul(complex_double(std::complex<double>(2.0, 3.0)), x), y));
