...
TEST_CASE("Constructing MExprPoly", "[MExprPoly]") {
  ...
  RCP<const MExprPoly> p1 = MExprPoly::from_dict(
      {x, y}, {{{1, 1}, a}, {{1, 2}, negB}, {{2, 1}, num1}, {{0, 1}, negNum}});
  RCP<const MExprPoly> pprime = MExprPoly::from_dict(
      {y, x}, {{{1, 1}, a}, {{1, 2}, negB}, {{2, 1}, num1}, {{0, 1}, negNum}});
  ...
  RCP<const MExprPoly> p6 = MExprPoly::from_dict(
      {x, y},
  ...
  REQUIRE(eq(
      *p2->as_symbolic(),
      *add({mul(comp4.get_basic(), mul(pow(x, integer(3)), pow(y, integer(4)))),
...
TEST_CASE("Testing MExprPoly::__eq__(), __hash__, and compare", "[MExprPoly]") {
  ...
  RCP<const MExprPoly> p3 =
      MExprPoly::from_dict({x, y}, {{{2, 0}, sum + sum}, {{0, 2}, sum + sum}});
...
TEST_CASE("Testing MExprPoly::eval", "[MExprPoly]") {
  ...
  REQUIRE(p->eval(m2) ==
          expr1 * pow_ex(ex, 2) + expr2 * pow_ex(why, 2) +
              ...
              expr1 * ex * why + expr2 * why * zee + expr1 * ex + expr2 * why +
              expr3 * zee + expr4 +
...
TEST_CASE("Testing addition, subtraction, multiplication of "
  ...
  RCP<const MExprPoly> q22 =
      MExprPoly::from_dict({x, y}, {{{2, -1}, -1 * num1},
                                    ...
                                    {{0, 0}, comp4}});
  RCP<const MExprPoly> q3 =
      MExprPoly::from_dict({x, y}, {{{2, 1}, a * comp1},
                                    {{1, 1}, a * comp4},
