...
TEST_CASE("basic_to_poly UInt", "[b2poly]") {
  ...
  CHECK_THROWS_AS(from_basic<UIntPoly>(
...
TEST_CASE("basic_to_poly URat", "[b2poly]") {
  ...
  CHECK_THROWS_AS(from_basic<URatPoly>(
                      UExprPoly::from_vec(gen, {{symbol("y"), one, one}}), gen),
...
TEST_CASE("basic_to_poly UIntFlint", "[b2poly]") {
  ...
  CHECK_THROWS_AS(
      from_basic<UIntPolyFlint>(
          SymEngine::URatPolyPiranha::from_vec(gen, {{1_q, 2_q, 0_q, 4_q}}), x),
      SymEngineException);
#endif
...
TEST_CASE("basic_to_poly MExpr", "[b2poly]") {
  ...
  poly1 = from_basic<MExprPoly>(basic, gens);
  poly2 =
      MExprPoly::from_dict({x, y}, {{{1, 1}, one}, {{0, 1}, z}, {{1, 0}, z}});
  REQUIRE(eq(*poly1, *poly2));
  ...
  gens = {pi, E};
  poly1 = from_basic<MExprPoly>(basic, gens);
  poly2 = MExprPoly::from_dict({pi, E}, {{{1, 1}, add(one, z)}, {{2, 0}, one}});
  REQUIRE(eq(*poly1, *poly2));
}