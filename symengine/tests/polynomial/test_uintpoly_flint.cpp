...
TEST_CASE("Factors of UIntPolyFlint", "[UIntPolyFlint]") {
  ...
  if (__FLINT_RELEASE <= 20502) {
    CHECK_THROWS_AS(factors(*UIntPolyFlint::from_dict(x, {{0, 1_z}, {1, 2_z}})),
                    std::runtime_error);
  } else {
    auto factorcheck = [](
      ...
      auto it =
          find_if(fac.begin(), fac.end(),
                  [&fac1](const std::pair<RCP<const UIntPolyFlint>, long> &s) {
                    return ((s.first->compare(*fac1.first) == 0) and
                            (s.second == fac1.second));
                  });
      return (it != fac.end());
    };

    auto fac =
        factors(*UIntPolyFlint::from_dict(x, {{0, 1_z}, {1, 2_z}})); // 2*x + 1
    REQUIRE(fac.size() == 1);
    REQUIRE(factorcheck(
        fac,
