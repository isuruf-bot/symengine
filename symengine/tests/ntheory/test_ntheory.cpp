...
TEST_CASE("test_factor_lehman_method(): ntheory", "[ntheory]") {
  ...
  REQUIRE((divides(*i21, *f) and not eq(*f, *i1) and
           not eq(*f, *i21))); // Lehman's method returns only a proper
...
TEST_CASE("test_factor_pollard_pm1_method(): ntheory", "[ntheory]") {
  ...
  REQUIRE(
      (factor_pollard_pm1_method(outArg(f), *i121) == 0 or divides(*i121, *f)));
  REQUIRE(
      (factor_pollard_pm1_method(outArg(f), *i122) == 0 or divides(*i122, *f)));
  REQUIRE(
      (factor_pollard_pm1_method(outArg(f), *i900) == 0 or divides(*i900, *f)));
  REQUIRE((factor_pollard_pm1_method(outArg(f), *i1001, 20) == 0 or
           divides(*i1001, *f)));
  REQUIRE((factor_pollard_pm1_method(outArg(f), *i1850) == 0 or
           divides(*i1850, *f)));
...
TEST_CASE("test_factor_pollard_rho_method(): ntheory", "[ntheory]") {
  ...
  REQUIRE(
      (factor_pollard_rho_method(outArg(f), *i121) == 0 or divides(*i121, *f)));
  REQUIRE(
      (factor_pollard_rho_method(outArg(f), *i122) == 0 or divides(*i122, *f)));
  REQUIRE(
      (factor_pollard_rho_method(outArg(f), *i900) == 0 or divides(*i900, *f)));
  REQUIRE((factor_pollard_rho_method(outArg(f), *i1001) == 0 or
           divides(*i1001, *f)));
  REQUIRE((factor_pollard_rho_method(outArg(f), *i1850) == 0 or
           divides(*i1850, *f)));
...
TEST_CASE("test_primitive_root(): ntheory", "[ntheory]") {
  ...
  bool same =
      std::equal(v.begin(), v.end(), roots.begin(), SymEngine::RCPBasicKeyEq());
  REQUIRE(same == true);
...
TEST_CASE("test_quadratic_residues(): ntheory", "[ntheory]") {
  ...
  vec_integer_class i5 = {integer_class(0), integer_class(1), integer_class(4)};
  vec_integer_class i7 = {integer_class(0), integer_class(1), integer_class(2),
                          integer_class(4)};
