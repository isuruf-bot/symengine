...
struct Expr1 {
  ...
  vec_basic expr() {
    ...
    r[0] = sin(
        add(vec[0], cos(add(mul(vec[1], vec[2]), pow(vec[0], integer(2))))));
...
struct Expr2 {
  ...
  template <typename Real> static void compiled_expr(Real *d, const Real *v) {
...
template <typename Real> struct NativeVisitor {
  void (*f)(Real *, const Real *){nullptr};
  void call(Real *d, const Real *v) { f(d, v); }
};
